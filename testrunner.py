...
def run_pytest_cmd(args, te_tag=None, parallel_exe=False, env=None, re_execution=False):
    ...
    if re_execution:
        te_tag = None
        report_name = "--html=log/re_non_parallel_" + te_id + "_" + datetime.utcnow().strftime('%Y-%m-%d_%H:%M:%S.%f') + args.html_report
        ...
        if parallel_exe and not args.force_serial_run:
            ...
            report_name = "--html=log/non_parallel_" + te_id + "_" + datetime.utcnow().strftime('%Y-%m-%d_%H:%M:%S.%f') + args.html_report
            cmd_line = ["pytest", is_parallel, is_distributed,
                        log_level, report_name, force_serial_run]
