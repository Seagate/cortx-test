...
def run(opts: dict) -> None:
    ...
    create_test_map(rev_tag_map, test_map, base_components_marks, meta_data,
                    skip_marks, skip_test)
    ...
    _producer = Thread(target=producer.server, args=(topic, work_queue, finish))
    _producer.setDaemon()
    _producer.start()
...
def main(argv=None):
    ...
    try:
        ...
        sys.stderr.write(program_name + ": " + repr(fault) + "\n")
        sys.stderr.write(indent + "  for help use --help")
        return 2


if __name__ == "__main__":
    emutex, flag = system_utils.FileLock(LCK_FILE)
