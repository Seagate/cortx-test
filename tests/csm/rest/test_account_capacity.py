...
class TestAccountCapacity():
    ...
    def test_33369(self):
        ...
        with Pool(s3acct_cnt) as pool:
            resp = pool.map(self.acc_capacity.perform_io_validate_data_usage,
                            zip(data, repeat(NORMAL_UPLOAD_SIZES_IN_MB),
                                repeat(validate_data_usage)))
        assert_utils.assert_true(all(resp),
                                 "Failure in Validating data usage for multiple S3 accounts")
