...
def teardown_function(function):
    ...
    assert_utils.assert_equals(resp[0], True)
    ...
    if my_users:
        for user in my_users:
            LOGGER.info("Deleting CSM users {0}".format(user))
            resp = CSM_USER.delete_csm_user(user_name=user)
            LOGGER.info("Deleted CSM users {0}".format(user))
...
def test_1143():
    ...
    LOGGER.info("Creating csm user with name {0}".format(user_name))
    ...
    LOGGER.info("Created csm user with name {0}".format(user_name))
    LOGGER.info(
