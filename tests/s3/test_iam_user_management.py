...
class TestIAMUserManagement:
    ...
    def test_32397(self):
        ...
        self.iam_user = "iamuser-{}".format(perf_counter_ns())
        resp = self.auth_obj.create_iam_user(
            self.iam_user, self.iam_password, S3_access_key, S3_secret_key)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 3: Try to create duplicate s3iamuser using direct REST API call")
        resp = self.auth_obj.create_iam_user(
            self.iam_user, self.iam_password, S3_access_key, S3_secret_key)
        assert_utils.assert_false(resp[0], resp[1])
        self.log.info("Step 4: list s3iamusers using access/secret key using direct REST API call.")
        resp = self.auth_obj.list_iam_users(S3_access_key, S3_secret_key)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 5: Delete s3iamuser using access/secret key using direct REST API call")
        resp = self.auth_obj.delete_iam_user(self.iam_user, S3_access_key, S3_secret_key)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 6: Delete s3 Account")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32286(self):
        ...
        for access_key in IAM_access_keys:
            ...
            resp = self.auth_obj.create_custom_iam_user(
                self.iam_user, self.iam_password, S3_access_key, S3_secret_key, access_key, secret)
            assert_utils.assert_false(resp[0], resp[1])
        self.log.info("Step 3: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32285(self):
        ...
        for secret_key in IAM_secret_keys:
            ...
            self.iam_user = "iamuser-{}".format(perf_counter_ns())
            access = self.iam_user.ljust(cons.Rest.IAM_ACCESS_LL, "d")
            resp = self.auth_obj.create_custom_iam_user(
                self.iam_user, self.iam_password, S3_access_key, S3_secret_key, access, secret_key)
            assert_utils.assert_false(resp[0], resp[1])
        self.log.info("Step 3: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32284(self):
        ...
        secret = config_utils.gen_rand_string(length=cons.Rest.IAM_SECRET_LL)
        resp = self.auth_obj.create_custom_iam_user(
            self.iam_user, self.iam_password, S3_access_key, S3_secret_key, IAM_secret_key=secret)
        assert_utils.assert_false(resp[0], resp[1])
        self.log.info("Step 3: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32283(self):
        ...
        access = self.iam_user.ljust(cons.Rest.IAM_ACCESS_LL, "d")
        resp = self.auth_obj.create_custom_iam_user(
            self.iam_user, self.iam_password, S3_access_key, S3_secret_key, IAM_access_key=access)
        assert_utils.assert_false(resp[0], resp[1])
        self.log.info("Step 3: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32282(self):
        ...
        S3_secret_key = resp[1]["secret_key"]
        self.log.info("Step 2: Create s3iamuser using direct REST API call")
        self.iam_user1 = "iamuser-{}".format(perf_counter_ns())
        secret1 = config_utils.gen_rand_string(length=cons.Rest.IAM_SECRET_LL)
        access1 = self.iam_user1.ljust(cons.Rest.IAM_ACCESS_LL, "d")
        resp = self.auth_obj.create_custom_iam_user(
            self.iam_user1, self.iam_password, S3_access_key, S3_secret_key, access1, secret1)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 3: Try to create s3iamuser using direct REST API call")
        self.iam_user2 = "iamuser-{}".format(perf_counter_ns())
        secret2 = config_utils.gen_rand_string(length=cons.Rest.IAM_SECRET_LL)
        resp = self.auth_obj.create_custom_iam_user(
            self.iam_user2, self.iam_password, S3_access_key, S3_secret_key, access1, secret2)
        assert_utils.assert_false(resp[0], resp[1])
        self.log.info("Step 4: Delete s3iamuser using access/secret key using direct REST API call")
        resp = self.auth_obj.delete_iam_user(self.iam_user1, S3_access_key, S3_secret_key)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 5: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32281(self):
        ...
        self.iam_user2 = "iamuser-{}".format(perf_counter_ns())
        access2 = self.iam_user2.ljust(cons.Rest.IAM_ACCESS_LL, "d")
        resp = self.auth_obj.create_custom_iam_user(
            self.iam_user2, self.iam_password, S3_access_key, S3_secret_key, access2, secret1)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 4: Delete s3iamuser using access/secret key using direct REST API call")
        resp = self.auth_obj.delete_iam_user(self.iam_user1, S3_access_key, S3_secret_key)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 5: Delete s3iamuser using access/secret key using direct REST API call")
        resp = self.auth_obj.delete_iam_user(self.iam_user2, S3_access_key, S3_secret_key)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 6: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32694(self):
        ...
        access = self.iam_user.ljust(cons.Rest.IAM_ACCESS_LL, "d")
        resp = self.auth_obj.create_custom_iam_user(
            self.iam_user, self.iam_password, S3_access_key, S3_secret_key, access, S3_secret_key)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 3: Delete s3iamuser using access/secret key using direct REST API call")
        resp = self.auth_obj.delete_iam_user(self.iam_user, S3_access_key, S3_secret_key)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 4: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info(
    ...
    def test_32693(self):
        ...
        secret = config_utils.gen_rand_string(length=cons.Rest.IAM_SECRET_LL)
        resp = self.auth_obj.create_custom_iam_user(
            self.iam_user, self.iam_password, S3_access_key, S3_secret_key, S3_access_key, secret)
        assert_utils.assert_false(resp[0], resp[1])
        self.log.info("Step 3: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32705(self):
        ...
        self.log.info("Step 1: Create s3 Account")
        self.acc_name1 = self.s3_user.format(perf_counter_ns())
        self.email_id1 = "{}@seagate.com".format(self.acc_name1)
        resp = self.rest_obj.create_s3_account(self.acc_name1, self.email_id1, self.acc_password1)
        assert_utils.assert_true(resp[0], resp[1])
        S3_access_key1 = resp[1]["access_key"]
        self.log.info("Step 2: Create another s3 Account")
        self.acc_name2 = self.s3_user.format(perf_counter_ns())
        self.email_id2 = "{}@seagate.com".format(self.acc_name2)
        resp = self.rest_obj.create_s3_account(self.acc_name2, self.email_id2, self.acc_password2)
        assert_utils.assert_true(resp[0], resp[1])
        ...
        secret = config_utils.gen_rand_string(length=cons.Rest.IAM_SECRET_LL)
        resp = self.auth_obj.create_custom_iam_user(
            self.iam_user, self.iam_password, S3_access_key2, S3_secret_key2,
            S3_access_key1, secret)
        assert_utils.assert_false(resp[0], resp[1])
        self.log.info("Step 4: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name1)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 5: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name2)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info(
    ...
    def test_32704(self):
        ...
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 4: Delete s3iamuser using access/secret key using direct REST API call")
        resp = self.auth_obj.delete_iam_user(self.iam_user, S3_access_key2, S3_secret_key2)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 5: Delete s3 account.")
        resp = self.rest_obj.delete_s3_account(self.acc_name1)
        assert_utils.assert_true(resp[0], resp[1])
