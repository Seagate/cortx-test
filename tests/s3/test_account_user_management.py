...
class TestAccountUserManagement:
    ...
    def teardown_class(cls):
        """
        This function will be invoked after completion of all test case.
    ...
    def setup_method(self):
        ...
        self.ldap_user = CMN_CFG["ldap_username"]
        self.ldap_passwd = CMN_CFG["ldap_passwd"]
    ...
    def teardown_method(self):
        ...
        users_list = [user["UserName"]
                      for user in all_users if self.acc_user_config["user_name"] in user["UserName"]]
        LOGGER.info("IAM users: {0}".format(users_list))
        if users_list:
            ...
            for user in users_list:
                ...
                try:
                    LOGGER.info("Deleting a user: {}".format(user))
        ...
        if all_acc:
            LOGGER.info("Accounts to delete: {0}".format(all_acc))
            for acc in all_acc:
                try:
                    LOGGER.info("Deleting {0} account".format(acc))
                    iam_obj.reset_access_key_and_delete_account_s3iamcli(acc)
                    LOGGER.info("Deleted {0} account".format(acc))
                except CTException as error:
                    LOGGER.info(error)
    ...
    def test_create_new_account_1968(self):
        ...
        LOGGER.info(
            "Step 1: Creating a new account with name {0}".format(account_name))
    ...
    def test_delete_account_1970(self):
        ...
        LOGGER.info(
            "Step 2: Deleting account with name {0}".format(account_name))
    ...
    def test_create_n_number_of_account_1971(self):
        ...
        LOGGER.info("Step 1: Creating {0} accounts".format(total_account))
        # Defining list.
        account_list, access_keys, secret_keys = list(), list(), list()
        acc_name = self.acc_user_config["account_name"]
        LOGGER.info("account prefix: {}".format(acc_name))
        for n in range(total_account):
            ...
            LOGGER.info("email id: {}".format(email_id))
        ...
        LOGGER.info("Created {0} accounts".format(total_account))
        LOGGER.info(
            "Step 2: Verifying {0} accounts are created by listing accounts".format(total_account))
        list_of_accounts = iam_obj.list_accounts_s3iamcli(
            self.ldap_user, self.ldap_passwd)
        assert list_of_accounts[0], list_of_accounts[1]
        new_accounts = [acc["AccountName"] for acc in list_of_accounts[1]]
        ...
        LOGGER.info(
            "Verified {0} accounts are created by listing accounts".format(total_account))
    ...
    def test_create_new_account_with_existing_name_1972(self):
        ...
        LOGGER.info(resp)
        assert resp[0], resp[1]
        LOGGER.info("Created a new account with name {0}".format(account_name))
        LOGGER.info(
    ...
    def test_crud_operations_with_valid_cred_1973(self):
        ...
        LOGGER.info("secret key: {}".format(secret_key))
        ...
        assert resp[0], resp[1]
        LOGGER.info(
            "Bucket with name {0} is created successfully".format(bucket_name))
        obj_name = TEST_CFG["test_8533"]["obj_name"]
        LOGGER.info("Object name: {}".format(obj_name))
        resp = create_file(
            self.acc_user_config["test_file_path"],
            TEST_CFG["test_8533"]["file_size"])
        LOGGER.info(resp)
        LOGGER.info(
            "Putting object {0} to bucket {1}".format(
                obj_name, bucket_name))
        resp = s3_user_obj.put_object(
            bucket_name, obj_name, self.acc_user_config["test_file_path"])
        assert resp[0], resp[1]
        LOGGER.info(
            "Object {0} successfully put to bucket {0}".format(
                obj_name, bucket_name))
        LOGGER.info("Downloading object from bucket {0}".format(bucket_name))
        ...
        LOGGER.info(
            "Downloading object from bucket {0} successfully".format(bucket_name))
        LOGGER.info(
    ...
    def test_crud_operations_with_invalid_cred_1974(self):
        ...
        LOGGER.info("user_access_key: {}".format(user_access_key))
        LOGGER.info("user_secret_key: {}".format(user_secret_key))
        s3_user_obj = S3TestLib(
            access_key=user_access_key,
            secret_key=user_secret_key)
        LOGGER.info(
        ...
        LOGGER.info("Bucket with name {0} is not created".format(bucket_name))
        ...
        try:
            ...
            assert err_message not in error.message, error.message
        LOGGER.info(
            "Could not put object {0} to bucket {0}".format(
                obj_name, bucket_name))
        LOGGER.info("Downloading object from bucket {0}".format(bucket_name))
        try:
            resp = s3_user_obj.object_download(
        ...
        LOGGER.info(
            "Could not download object from bucket {0}".format(bucket_name))
        LOGGER.info(
    ...
    def test_create_new_user_from_current_account_2076(self):
        ...
        account_name = f'{self.acc_user_config["account_name"]}_{str(int(time.time()))}'
        user_name = f'{self.acc_user_config["user_name"]}_{str(int(time.time()))}'
        LOGGER.info("account_name: {}".format(account_name))
        ...
        LOGGER.info(resp)
        LOGGER.info("Users_List {}".format(resp[1]))
        assert resp[0], resp[1]
    ...
    def test_create_n_number_of_users_2080(self):
        ...
        LOGGER.info("Step 1: Create new {} account".format(total_users))
        account_name = f'{self.acc_user_config["account_name"]}_{str(int(time.time()))}'
        user_name = f'{self.acc_user_config["user_name"]}_{str(int(time.time()))}'
        resp = self.create_account(account_name)
        assert resp[0], resp[1]
        ...
        LOGGER.info("Step 2: Creating {0} users".format(total_users))
        for n in range(total_users):
            my_user_name = f"{user_name}{n}"
            LOGGER.info("Creating user with name {0}".format(my_user_name))
            resp = iam_obj.create_user_using_s3iamcli(
                my_user_name, access_key, secret_key)
            assert resp[0], resp[1]
            LOGGER.info("Created user with name {0}".format(my_user_name))
        LOGGER.info("Step 2: Created {0} users".format(total_users))
        LOGGER.info("Verifying {0} users are created".format(total_users))
        new_iam_obj = IamTestLib(access_key=access_key, secret_key=secret_key)
        list_of_users = new_iam_obj.list_users()[1]
        LOGGER.info(list_of_users)
        LOGGER.info("Number of users : {}".format(len(list_of_users)))
        assert resp[0], resp[1]
        assert len(list_of_users) >= total_users, list_of_users[1]
        LOGGER.info("Verified {0} users are created".format(total_users))
    ...
    def test_create_user_with_existing_name_2081(self):
        ...
        user_name = f'{self.acc_user_config["user_name"]}_{str(int(time.time()))}'
        LOGGER.info("Step 1: Creating user with name {0}".format(user_name))
        resp = iam_obj.create_user(user_name)
        assert resp[0], resp[1]
        LOGGER.info("Created user with name {0}".format(user_name))
        LOGGER.info(
            "Step 2: Creating user with existing name {0}".format(user_name))
        try:
            resp = iam_obj.create_user(user_name)
            LOGGER.info(resp)
            assert not resp[0], resp[1]
        ...
        LOGGER.info(
            "Could not create user with existing name {0}".format(user_name))
    ...
    def test_create_max_accesskey_with_existing_user_name_2087(self):
        ...
        LOGGER.info("Created a user with name {0}".format(user_name))
        LOGGER.info(
            "Step 2: Creating {0} access keys for user {1}".format(
                test_8686_cfg["max_access_keys"],
                user_name))
    ...
    def test_update_login_profile_2088(self):
        ...
        LOGGER.info("Created a user with name {0}".format(user_name))
        LOGGER.info(
            "Step 2: Creating login profile for user {0}".format(user_name))
        resp = iam_obj.create_user_login_profile(
            user_name, test_8687_cfg["password"], test_8687_cfg["password_reset"])
        assert resp[0], resp[1]
        LOGGER.info("Created login profile for user {0}".format(user_name))
        LOGGER.info(
            "Step 3: Updating login profile for user {0}".format(user_name))
        resp = iam_obj.update_user_login_profile(
            user_name,
            test_8687_cfg["new_password"],
            test_8687_cfg["password_reset"])
        assert resp[0], resp[1]
        LOGGER.info("Updated login profile for user {0}".format(user_name))
    ...
    def test_ssl_certificate_2090(self):
        ...
        s3hobj.copy_s3server_file(
            ca_cert_path, test_8689_cfg["local_cert_path"])
    ...
    def test_check_ssl_certificate_present_2091(self):
        ...
        ca_cert_path = const.CA_CERT_PATH
        LOGGER.info(
            "Step 1: Checking if {0} file exists on server".format(ca_cert_path))
        resp = s3hobj.is_s3_server_path_exists(ca_cert_path)
        assert resp, "certificate path not present: {}".format(ca_cert_path)
        LOGGER.info(
            "Verified that {0} file exists on server".format(ca_cert_path))
    ...
    def test_change_pwd_for_iam_user_2092(self):
        ...
        LOGGER.info("Created login profile for user {0}".format(user_name))
        LOGGER.info(
            "Step 3: Creating access key for user {0}".format(user_name))
        resp = iam_obj.create_access_key(user_name)
        LOGGER.info(resp)
        assert resp[0], resp[1]
        LOGGER.info("Created access key for user {0}".format(user_name))
        access_key = resp[1]["AccessKey"]["AccessKeyId"]
        secret_key = resp[1]["AccessKey"]["SecretAccessKey"]
        LOGGER.info("Step 4: Changing password for {0} user".format(user_name))
        ...
        LOGGER.info(resp)
        assert resp[0], resp[1]
        LOGGER.info("Changed password for {0} user".format(user_name))
    ...
    def test_create_user_account_and_check_arn_4625(self):
        ...
        LOGGER.info("Created a new account with name {0}".format(account_name))
        LOGGER.info("Step 2: Creating a user with name {0}".format(user_name))
        resp = iam_obj.create_user_using_s3iamcli(
            user_name, access_key, secret_key)
        assert resp[0], resp[1]
        LOGGER.info("Created a user with name {0}".format(user_name))
        LOGGER.info(
            "Step 3: Verifying ARN format of user {0}".format(user_name))
        arn_format = test_4625_cfg["arn_str"].format(account_id, user_name)
        assert arn_format == resp[1]["ARN"], test_4625_cfg["err_message"]
        LOGGER.info(
            "Step 3: Verified ARN format of user {0} successfully".format(user_name))
        LOGGER.info(
