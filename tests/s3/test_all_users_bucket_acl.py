...
class TestAllUsersBucketAcl:
    ...
    def setup(self):
        ...
        if self.bucket_name in bucket_list:
            self.acl_obj.put_bucket_acl(self.bucket_name, grant_full_control=self.group_uri)
            resp = self.s3_test_obj.delete_bucket(self.bucket_name, force=True)
            assert_utils.assert_true(resp[0], resp[1])
        self.log.info("ENDED: Teardown operations")

    def create_bucket_put_object(self, bucket_name, obj_name, file_path, mb_count):
        """
        Function creates a bucket and uploads an object to the bucket.
        ...
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Uploaded an object %s to bucket %s", obj_name, bucket_name)
    ...
    def test_375(self):
        """Check listing of objects in bucket without Auth when AllUsers have READ permission."""
        self.log.info("STARTED: Check listing of objects in bucket without "
                      "Authentication when AllUsers have READ permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name, self.obj_name, self.test_file_path,
                                      self.mb_count)
        ...
        resp = s3_utils.poll(self.acl_obj.get_bucket_acl, self.bucket_name,
                             condition='{}[1][1][0]["Permission"]=="READ"')
        assert_utils.assert_true(resp[0], resp[1])
        ...
        self.log.info("ENDED: Check listing of objects in bucket without Authentication when "
    ...
    def test_376(self):
        ...
        self.log.info("STARTED: Put an object in bucket without Authentication"
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to put an object to a bucket from other unsigned user")
        try:
    ...
    @CTFailOn(error_handler)
    def test_377(self):
        """Delete an object from bucket without Authentication when AllUsers have READ perm."""
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to delete an object from other unsigned user")
        try:
    ...
    @CTFailOn(error_handler)
    def test_378(self):
        """Read an object ACL from bucket without Authentication when AllUsers have READ perm."""
    ...
    @CTFailOn(error_handler)
    def test_379(self):
        """Read a bucket ACL of a bucket without Authentication when AllUsers have READ perm."""
        self.log.info("STARTED: Read a bucket ACL of a bucket without Authentication "
                      "when AllUsers have READ permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name, self.obj_name, self.test_file_path,
                                      self.mb_count)
    ...
    @CTFailOn(error_handler)
    def test_380(self):
        """Update a bucket ACL for a bucket without Auth when AllUsers have READ permission."""
        self.log.info("STARTED: Update a bucket ACL for a bucket without Authentication "
                      "when AllUsers have READ permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name, self.obj_name, self.test_file_path,
                                      self.mb_count)
    ...
    @CTFailOn(error_handler)
    def test_381(self):
        """Update an object ACL from bucket without Auth when AllUsers have READ permission."""
        self.log.info("STARTED: Update an object ACL from bucket without Authentication "
                      "when AllUsers have READ permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name, self.obj_name, self.test_file_path,
                                      self.mb_count)
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Updating object's ACL from other unsigned user")
        try:
            resp = self.no_auth_obj.put_object_canned_acl(self.bucket_name, self.obj_name,
                                                          acl="private")
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
            assert_utils.assert_in("AccessDenied", error.message, error.message)
    ...
    @CTFailOn(error_handler)
    def test_382(self):
        """Listing of objects in bucket without Auth when AllUsers have WRITE permission."""
        self.log.info("STARTED: Listing of objects in bucket without Authentication "
                      "when AllUsers have WRITE permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
        self.log.info("Step 1: Created a bucket and uploaded an object")
        ...
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Listing objects in bucket from other unsigned user")
        try:
    ...
    @CTFailOn(error_handler)
    def test_383(self):
        """Create an object in bucket without Auth when AllUsers have WRITE permission."""
        self.log.info("STARTED: Create an object in bucket without Authentication when "
                      "AllUsers have WRITE permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
        ...
        resp = s3_utils.poll(self.no_auth_obj.put_object, self.bucket_name, self.new_obj_name,
                             self.test_file_path)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 4: An object %s is put to bucket %s successfully",
                      self.new_obj_name, self.bucket_name)
        self.log.info("Step 5: Setting the bucket ACL to 'private'")
        resp = self.acl_obj.put_bucket_acl(self.bucket_name, acl="private")
        assert_utils.assert_true(resp[0], resp[1])
    ...
    @CTFailOn(error_handler)
    def test_384(self):
        """Delete an object from bucket without Auth when AllUsers have WRITE permission."""
    ...
    @CTFailOn(error_handler)
    def test_385(self):
        """Read an object ACL from bucket without Auth when AllUsers have WRITE permission."""
        self.log.info("STARTED: Read an object ACL from bucket without Authentication"
                      " when AllUsers have WRITE permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name,
    ...
    @CTFailOn(error_handler)
    def test_386(self):
        """Read a bucket ACL from bucket without Auth when AllUsers have WRITE permission."""
        self.log.info("STARTED: Read a bucket ACL from bucket without Authentication"
                      " when AllUsers have WRITE permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
    ...
    @CTFailOn(error_handler)
    def test_387(self):
        """Update a bucket ACL from bucket without Auth when AllUsers have WRITE permission."""
        self.log.info("STARTED: Update a bucket ACL from bucket without Authentication"
                      " when AllUsers have WRITE permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
        self.log.info("Step 1: Created a bucket and uploaded an object to bucket")
        self.log.info("Step 2: Changing bucket permission to AllUsers WRITE")
        resp = self.acl_obj.put_bucket_acl(self.bucket_name, grant_write=self.group_uri)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 2: Changed bucket permission to AllUsers WRITE")
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to change bucket ACL to private from other unsigned user")
        try:
            resp = self.no_auth_obj.put_bucket_acl(self.bucket_name, acl="private")
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
    ...
    @CTFailOn(error_handler)
    def test_388(self):
        """Update an object ACL from bucket without Auth when AllUsers have WRITE permission."""
        self.log.info("STARTED: Update an object ACL from bucket without Authentication "
                      "when AllUsers have WRITE permission")
        self.log.info("Step 1: Creating a bucket and uploading an object")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to change object ACL from other unsigned user")
        try:
            resp = self.no_auth_obj.put_object_canned_acl(self.bucket_name, self.obj_name,
                                                          acl="private")
            assert_utils.assert_false(resp[0], resp[1])
    ...
    @CTFailOn(error_handler)
    def test_389(self):
        """Check listing of objects in bucket without Auth when AllUsers have READ_ACP perm."""
        self.log.info("STARTED: Check listing of objects in bucket without Authentication"
                      " when AllUsers have READ_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
        self.log.info("Step 1: Created a bucket and uploaded an object to bucket")
        self.log.info("Step 2: Changing bucket permission to AllUsers READ_ACP")
        resp = self.acl_obj.put_bucket_acl(self.bucket_name, grant_read_acp=self.group_uri)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 2: Changed bucket permission to AllUsers READ_ACP")
        self.log.info("Step 3: Verifying bucket permission is changed")
        resp = self.acl_obj.get_bucket_acl(self.bucket_name)
        assert_utils.assert_true(resp[0], resp[1])
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
    ...
    @CTFailOn(error_handler)
    def test_390(self):
        """Create an object in bucket without Auth when AllUsers have READ_ACP permission."""
        self.log.info("STARTED: Create an object in bucket without Authentication"
                      " when AllUsers have READ_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to upload an object from other unsigned user")
        try:
            resp = self.no_auth_obj.put_object(self.bucket_name, self.new_obj_name,
                                               self.test_file_path)
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
            assert_utils.assert_in("AccessDenied", error.message, error.message)
    ...
    @CTFailOn(error_handler)
    def test_391(self):
        """Delete an object from bucket without Auth when AllUsers have READ_ACP permission."""
        self.log.info("STARTED: Delete an object from bucket without Authentication "
                      "when AllUsers have READ_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4:Trying to delete existing object from other unsigned user")
        try:
            resp = self.no_auth_obj.delete_object(self.bucket_name, self.obj_name)
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
    ...
    @CTFailOn(error_handler)
    def test_392(self):
        """Read an object ACL from bucket without Auth when AllUsers have READ_ACP permission."""
        self.log.info("STARTED: Read an object ACL from bucket without Authentication "
                      "when AllUsers have READ_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to read existing object's ACL from other unsigned user")
        try:
            resp = self.no_auth_obj.get_object_acl(self.bucket_name, self.obj_name)
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
    ...
    @CTFailOn(error_handler)
    def test_393(self):
        """Read a bucket ACL from bucket without Auth when AllUsers have READ_ACP permission."""
        self.log.info("STARTED: Read a bucket ACL from bucket without Authentication"
                      " when AllUsers have READ_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to read bucket's ACL from other unsigned user")
        resp = self.no_auth_obj.get_bucket_acl(self.bucket_name)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 4: Read bucket's ACL from other unsigned user successfully")
        self.log.info("ENDED: Read a bucket ACL from bucket without Authentication when"
    ...
    @CTFailOn(error_handler)
    def test_394(self):
        """Update a bucket ACL from bucket without Auth when AllUsers have READ_ACP permission."""
        self.log.info("STARTED: Update a bucket ACL from bucket without Authentication"
                      " when AllUsers have READ_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to update bucket's ACL from other unsigned user")
        try:
            resp = self.no_auth_obj.put_bucket_acl(self.bucket_name, acl="private")
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
    ...
    @CTFailOn(error_handler)
    def test_395(self):
        """Update an object ACL from bucket without Auth when AllUsers have READ_ACP permission."""
        self.log.info("STARTED: Update an object ACL from bucket without Authentication "
                      "when AllUsers have READ_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "READ_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Trying to update object's ACL from other unsigned user")
        try:
    ...
    @CTFailOn(error_handler)
    def test_396(self):
        """Check listing of objects in bucket without Auth when AllUsers have WRITE_ACP perm."""
        self.log.info("STARTED: Check listing of objects in bucket without Authentication"
        ...
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
        self.log.info("Step 1: Created a bucket and uploaded an object to bucket")
        self.log.info("Step 2: Changing bucket permission to AllUsers WRITE_ACP")
        resp = self.acl_obj.put_bucket_acl(self.bucket_name, grant_write_acp=self.group_uri)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 2: Changed bucket permission to AllUsers WRITE_ACP")
        self.log.info("Step 3: Verifying bucket permission is changed")
        resp = self.acl_obj.get_bucket_acl(self.bucket_name)
        assert_utils.assert_true(resp[0], resp[1])
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Getting list of objects in bucket from other unsigned user")
        try:
            resp = self.no_auth_obj.object_list(self.bucket_name)
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
    ...
    @CTFailOn(error_handler)
    def test_397(self):
        """Create an object in bucket without Auth when AllUsers have WRITE_ACP permission."""
        self.log.info("STARTED: Create an object in bucket without Authentication "
                      "when AllUsers have WRITE_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Uploading an object to a bucket from other unsigned user")
        try:
    ...
    @CTFailOn(error_handler)
    def test_398(self):
        """Delete an object from bucket without Auth when AllUsers have WRITE_ACP permission."""
        self.log.info("STARTED: Delete an object from bucket without Authentication when "
                      "AllUsers have WRITE_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Deleting an object to a bucket from other unsigned user")
        try:
            resp = self.no_auth_obj.delete_object(self.bucket_name, self.obj_name)
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
    ...
    @CTFailOn(error_handler)
    def test_399(self):
        """Read an object ACL from bucket without Auth when AllUsers have WRITE_ACP permission."""
        self.log.info("STARTED: Read an object ACL from bucket without Authentication when "
                      "AllUsers have WRITE_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Reading object ACL through other unsigned account")
        try:
            resp = self.no_auth_obj.get_object_acl(self.bucket_name, self.obj_name)
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
    ...
    @CTFailOn(error_handler)
    def test_400(self):
        """Read a bucket ACL from bucket without Auth when AllUsers have WRITE_ACP permission."""
        self.log.info("STARTED: Read a bucket ACL from bucket without Authentication "
                      "when AllUsers have WRITE_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Reading bucket ACL through other unsigned account")
        try:
            resp = self.no_auth_obj.get_bucket_acl(self.bucket_name)
            assert_utils.assert_false(resp[0], resp[1])
        except CTException as error:
    ...
    @CTFailOn(error_handler)
    def test_401(self):
        """Update a bucket ACL from bucket without Auth when AllUsers have WRITE_ACP permission."""
        self.log.info("STARTED: Update a bucket ACL from bucket without Authentication"
                      " when AllUsers have WRITE_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Updating bucket ACL through other unsigned account")
        resp = self.no_auth_obj.put_bucket_acl(self.bucket_name, grant_full_control=self.group_uri)
        assert_utils.assert_true(resp[0], resp[1])
        resp = self.no_auth_obj.get_bucket_acl(self.bucket_name)
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "FULL_CONTROL", resp[1])
        self.log.info("Step 4: Updated bucket ACL through from other unsigned"
                      " user successfully")
        self.log.info("ENDED: Update a bucket ACL from bucket without Authentication"
                      " when AllUsers have WRITE_ACP permission")
    ...
    @CTFailOn(error_handler)
    def test_402(self):
        """Update an object ACL from bucket without Auth when AllUsers have WRITE_ACP permission."""
        self.log.info("STARTED: Update an object ACL from bucket without Authentication"
                      " when AllUsers have WRITE_ACP permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "WRITE_ACP", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Updating object ACL through other unsigned account")
        try:
            resp = self.no_auth_obj.put_object_canned_acl(
                self.bucket_name, self.obj_name, acl="private")
            assert_utils.assert_false(resp[0], resp[1])
    ...
    @CTFailOn(error_handler)
    def test_403(self):
        """Listing of objects in bucket without Auth when AllUsers have FULL_CONTROL permission."""
        self.log.info("STARTED: Listing of objects in bucket without Authentication "
        ...
        self.create_bucket_put_object(self.bucket_name,
                                      self.obj_name, self.test_file_path, self.mb_count)
        self.log.info("Step 1: Created a bucket and uploaded an object to bucket")
        self.log.info("Step 2: Changing bucket permission to AllUsers FULL_CONTROL")
        resp = self.acl_obj.put_bucket_acl(self.bucket_name, grant_full_control=self.group_uri)
        assert_utils.assert_true(resp[0], resp[1])
        self.log.info("Step 2: Changed bucket permission to AllUsers FULL_CONTROL")
        self.log.info("Step 3: Verifying bucket permission is changed")
        resp = self.acl_obj.get_bucket_acl(self.bucket_name)
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "FULL_CONTROL", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4: Listing objects in bucket through other unsigned account")
        resp = self.no_auth_obj.object_list(self.bucket_name)
        assert_utils.assert_true(resp[0], resp[1])
        assert_utils.assert_in(self.obj_name, str(resp[1]), resp[1])
        self.log.info("Step 4: Listed objects in bucket through other unsigned account successful")
        self.log.info("ENDED: Listing of objects in bucket without Authentication "
                      "when AllUsers have FULL_CONTROL permission")
    ...
    @CTFailOn(error_handler)
    def test_404(self):
        """Put an object in bucket without Auth when AllUsers have FULL_CONTROL permission."""
        self.log.info("STARTED: Put an object in bucket without Authentication "
                      "when AllUsers have FULL_CONTROL permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "FULL_CONTROL", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4:Uploading object in bucket through other unsigned account")
        resp = self.no_auth_obj.put_object(self.bucket_name, self.new_obj_name, self.test_file_path)
        assert_utils.assert_true(resp[0], resp[1])
        resp = self.no_auth_obj.object_list(self.bucket_name)
        assert_utils.assert_in(self.new_obj_name, str(resp[1]), resp[1])
        self.log.info("Step 4: Uploaded object in bucket through other unsigned account successful")
        self.log.info("ENDED: Put an object in bucket without Authentication "
                      "when AllUsers have FULL_CONTROL permission")
    ...
    @CTFailOn(error_handler)
    def test_405(self):
        """Delete an object from bucket without Auth when AllUsers have FULL_CONTROL perm."""
        self.log.info("STARTED: Delete an object from bucket without Authentication "
                      "when AllUsers have FULL_CONTROL permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
        ...
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "FULL_CONTROL", resp[1])
        self.log.info("Step 3: Verified bucket permission is changed")
        self.log.info("Step 4:Deleting obj from bucket through other unsigned account")
        resp = self.no_auth_obj.delete_object(self.bucket_name, self.obj_name)
        assert_utils.assert_true(resp[0], resp[1])
        resp = self.no_auth_obj.object_list(self.bucket_name)
        assert_utils.assert_not_in(self.obj_name, str(resp[1]), resp[1])
        self.log.info("Step 4:Deleted object from bucket through other unsigned account")
        self.log.info("ENDED: Delete an object from bucket without Authentication when"
    ...
    @CTFailOn(error_handler)
    def test_406(self):
        """Read an object ACL from bucket without Auth when AllUsers have FULL_CONTROL perm."""
        self.log.info("STARTED: Read an object ACL from bucket without Authentication"
                      " when AllUsers have FULL_CONTROL permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name,
    ...
    @CTFailOn(error_handler)
    def test_407(self):
        """Read a bucket ACL from bucket without Auth when AllUsers have FULL_CONTROL perm."""
        self.log.info("STARTED: Read a bucket ACL from bucket without Authentication "
                      "when AllUsers have FULL_CONTROL permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name, self.obj_name, self.test_file_path,
        ...
        resp = self.no_auth_obj.get_bucket_acl(self.bucket_name)
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "FULL_CONTROL", resp[1])
        self.log.info("Step 4: Read bucket ACL through other unsigned "
    ...
    @CTFailOn(error_handler)
    def test_408(self):
        """Update a bucket ACL from bucket without Auth when AllUsers have FULL-CONTROL perm."""
        self.log.info("STARTED: Update a bucket ACL from bucket without Authentication"
                      " when AllUsers have FULL_CONTROL permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name, self.obj_name, self.test_file_path,
        ...
        resp = self.acl_obj.get_bucket_acl(self.bucket_name)
        assert_utils.assert_equal(resp[1][1][0]["Permission"], "FULL_CONTROL", resp[1])
        self.log.info("Step 4: Updated bucket ACL through other unsigned account successfully")
        self.log.info("ENDED: Update a bucket ACL from bucket without Authentication"
                      " when AllUsers have FULL_CONTROL permission")
    ...
    @CTFailOn(error_handler)
    def test_409(self):
        """Update an object ACL from bucket without Auth when AllUsers have FULL_CONTROL perm."""
        self.log.info("STARTED: Update an object ACL from bucket without Authentication"
                      " when AllUsers have FULL_CONTROL permission")
        self.log.info("Step 1: Creating a bucket and uploading an object to bucket")
        self.create_bucket_put_object(self.bucket_name, self.obj_name, self.test_file_path,
