...
class TestObjectWorkflowOperations:
    ...
    def setup_method(self):
        ...
        self.s3_cmd_obj = s3_cmd_test_lib.S3CmdTestLib(endpoint_url=S3_CFG["s3_url"])
    ...
    def teardown_method(self):
        ...
        if path_exists(self.folder_path):
            ...
            self.log.info(
                ...
                self.folder_path,
                resp)
        bucket_list = self.s3_test_obj.bucket_list()
        pref_list = [
            each_bucket for each_bucket in bucket_list[
                1] if each_bucket.startswith(self.bkt_name_prefix)]
        self.s3_test_obj.delete_multiple_buckets(pref_list)
