...
OBJ_ACL_CONFIG = read_yaml("config/s3/test_object_acl.yaml")
...
class TestObjectACL:
    ...
    def teardown_method(self):
        ...
        if os.path.exists(OBJ_ACL_CONFIG["common_var"]["file_path"]):
            remove_file(OBJ_ACL_CONFIG["common_var"]["file_path"])

        all_accounts = IAM_TEST_OBJ.list_accounts_s3iamcli(
            self.ldap_user,
            self.ldap_password)[1]
        self.LOGGER.info("setup %s", all_accounts)
        iam_accounts = [acc["AccountName"]
                        for acc in all_accounts if
                        OBJ_ACL_CONFIG["common_var"]["acc_name_prefix"] in acc["AccountName"]]
        self.LOGGER.debug(iam_accounts)
        if iam_accounts:
            for acc in iam_accounts:
                self.LOGGER.debug("Deleting %s account", acc)
    ...
    def create_bucket_obj(self, bucket, obj, s3_test_obj=None):
        ...
        :return: None
        """
        file_path = OBJ_ACL_CONFIG["common_var"]["file_path"]
        global S3_OBJ
        S3_OBJ = s3_test_obj if s3_test_obj else S3_OBJ
        self.LOGGER.info("Step : Creating a bucket: %s", bucket)
        res = S3_OBJ.create_bucket(bucket)
        assert res[0], res[1]
        self.LOGGER.info("Step : Bucket is created: %s", bucket)
        self.LOGGER.info("Step : Creating a object: %s", obj)
        create_file(file_path,
                    OBJ_ACL_CONFIG["common_var"]["file_size"])
        res = S3_OBJ.put_object(bucket, obj, file_path)
        assert res[0], res[1]
        self.LOGGER.info("Step : Object is created: %s", obj)
    ...
    def create_acc_and_put_obj_acp(self, bkt_name, obj_name, test_cfg):
        ...
        self.LOGGER.info("Creating account with name %s and email_id %s",
        ...
        s3_obj_2 = result[1]
        self.LOGGER.info("Step: Put canned ACL for the Existing Object")
    ...
    def test_2874(self):
        ...
        obj = OBJ_ACL_CONFIG["common_var"]["object_name"].format(
            self.random_num)
        self.LOGGER.info("Bucket and Object : %s %s", bucket, obj)
        obj_acl = OBJ_ACL_CONFIG["test_9852"]["obj_acl"]
        self.create_bucket_obj(bucket, obj)
        self.LOGGER.info("Step 3: Getting the object ACL: %s", obj)
    ...
    def test_2875(self):
        ...
        obj = OBJ_ACL_CONFIG["common_var"]["object_name"].format(
            self.random_num)
        self.LOGGER.info("Step 1: Creating a bucket: %s", bucket)
        res = S3_OBJ.create_bucket(bucket)
        assert res[0], res[1]
        self.LOGGER.info("Step 1: Bucket is created: %s", bucket)
        self.LOGGER.info(
    ...
    def test_2876(self):
        ...
        try:
            ...
            assert OBJ_ACL_CONFIG["test_9853"]["err_msg"] in error.message, error.message
        self.LOGGER.info("Step 2: Object ACL resp is : %s", res)
        self.LOGGER.info(
            "verify that user able to download with empty key or not")
    ...
    def test_2877(self):
        ...
        self.LOGGER.info("User able to reupload object and get object acl")
        file_path = OBJ_ACL_CONFIG["common_var"]["file_path"]
        bucket = OBJ_ACL_CONFIG["common_var"]["bucket_name"].format(
            self.random_num)
        obj = OBJ_ACL_CONFIG["common_var"]["object_name"].format(
            self.random_num)
        self.create_bucket_obj(bucket, obj)
        self.LOGGER.info("Step 1: Re-Uploading Object : %s", obj)
        res = S3_OBJ.put_object(bucket, obj, file_path)
        assert res[0], res[1]
        self.LOGGER.info("Step 2: Object is re-uploaded status: %s", res)
        self.LOGGER.info(
    ...
    def test_2878(self):
        ...
        obj = OBJ_ACL_CONFIG["common_var"]["object_name"].format(
            self.random_num)
        self.create_bucket_obj(bucket, obj)
        self.LOGGER.info("Step 3: Deleting object %s", obj)
        res = S3_OBJ.delete_object(bucket, obj)
        assert res[0], res[1]
        self.LOGGER.info("Step 3: Object Deleted %s", obj)
        self.LOGGER.info(
        ...
        try:
            ...
            assert OBJ_ACL_CONFIG["test_9853"]["err_msg"] in error.message, error.message
        self.LOGGER.info("Step 4: Object ACL resp is : %s", res)
        self.LOGGER.info(
            "User should not get object acl when object was deleted")
    ...
    def test_get_obj_acl_mp(self):
        ...
        mupart_obj_path = OBJ_ACL_CONFIG["test_get_obj_acl_mp"]["mupart_obj_path"]
        self.LOGGER.info("Step 1: Creating a bucket: %s", bucket)
        res = S3_OBJ.create_bucket(bucket)
        assert res[0], res[1]
        self.LOGGER.info("Step 1: Bucket is created: %s", bucket)
        ...
        self.LOGGER.info("Step 3: Uploading multi parts for object")
        ...
        res = S3_MUPART_OBJ.upload_parts(
            ...
            mupart_obj_path)
        assert res[1], res[1]
        self.LOGGER.info("Step 3: Uploading parts for object in progress")
        upload_parts_list = res[1]
        res = S3_MUPART_OBJ.list_parts(upload_id, bucket, obj)
        assert res[0], res[1]
        self.LOGGER.info("Step 4: Complete the multipart upload process")
        res = S3_MUPART_OBJ.complete_multipart_upload(
            upload_id, upload_parts_list, bucket, obj)
        assert res[0], res[1]
        self.LOGGER.info("Step 4: Multipart upload completed")
        self.LOGGER.info(
        ...
        self.LOGGER.info("Clean Up Stage: Deleting all files created locally")
        self.LOGGER.info("Clean Up Stage: Done cleaning")
        self.LOGGER.info(
            "user should able to get Object acl when multipart object is uploaded")
    ...
    def test_2879(self):
        ...
        mupart_obj_path = OBJ_ACL_CONFIG["test_9857"]["mupart_obj_path"]
        self.LOGGER.info("Step 1: Creating a bucket: %s", bucket)
        res = S3_OBJ.create_bucket(bucket)
        assert res[0], res[1]
        self.LOGGER.info("Step 1: Bucket is created: %s", bucket)
        ...
        res = S3_MUPART_OBJ.list_parts(upload_id, bucket, obj)
        assert res[0], res[1]
        self.LOGGER.info("Step 3: Multipart upload list is : %s", res)
        self.LOGGER.info("Step 4: Complete the multipart upload")
        res = S3_MUPART_OBJ.complete_multipart_upload(
            upload_id, upload_parts_list, bucket, obj)
        assert res[0], res[1]
        self.LOGGER.info("Step 4: Completed multipart upload")
        self.LOGGER.info(
            "Step 5: Getting the object ACL for object : %s", obj)
        res = S3_ACL_OBJ.get_object_acl(bucket, obj)
        assert res[0], res[1]
    ...
    def test_2910(self):
        ...
        assert res[0], res[1]
        self.LOGGER.info("Step 1: Bucket is created: %s", bucket)
        self.LOGGER.info("Step 2: Creating file locally for upload")
        res = create_file(
            file_path, OBJ_ACL_CONFIG["test_9897"]["f_size"])
        assert res[0], res[1]
        self.LOGGER.info("Step 2: Completed creating files locally")
        self.LOGGER.info("Step 3: Uploading Object : %s", obj)
        res = S3_OBJ.object_upload(bucket, obj, file_path)
        assert res[0], res[1]
        self.LOGGER.info("Step 3: Completed Uploading for Object")
    ...
    def test_2912(self):
        ...
        mupart_obj_path = OBJ_ACL_CONFIG["test_9899"]["mupart_obj_path"]
        self.LOGGER.info("Step 1: Creating a bucket: %s", bucket)
        res = S3_OBJ.create_bucket(bucket)
        assert res[0], res[1]
        self.LOGGER.info("Step 1: Bucket is created: %s", bucket)
        ...
        res = S3_MUPART_OBJ.list_parts(upload_id, bucket, obj)
        assert res[0], res[1]
        self.LOGGER.info("Step 3: Uploading for Object in progress")
        self.LOGGER.info("Step 4: Aborting the multipart upload")
        res = S3_MUPART_OBJ.abort_multipart_all(bucket, obj)
        assert res[0], res[1]
        self.LOGGER.info("Step 4: Aborted the multipart upload")
        self.LOGGER.info(
            "Step 5: Getting the object ACL for object : %s", obj)
        try:
            S3_ACL_OBJ.get_object_acl(bucket, obj)
    ...
    def test_3210(self):
        ...
        self.create_bucket_obj(bucket, obj)
        self.LOGGER.info("Step 4: Added grantee to the object %s", obj)
        res = S3_ACL_OBJ.add_grantee(bucket, obj, canonical_id, permission)
        assert res[0], res[1]
        self.LOGGER.info("Step 4: Grantee added for the object %s", obj)
        self.LOGGER.info(
    ...
    def test_3213(self):
        ...
        result = self.create_s3iamcli_acc(account_name, email_id)
        canonical_id = result[0]
        self.LOGGER.info("Step 1: Completed creating account")
        self.create_bucket_obj(bucket, obj)
        self.LOGGER.info("Step 2: Put read permission to object %s", obj)
        res = S3_ACL_OBJ.add_grantee(bucket, obj, canonical_id, permission)
        assert res[0], res[1]
        self.LOGGER.info("Step 2: Added read permission to object")
        self.LOGGER.info(
    ...
    def test_3214(self):
        ...
        canonical_id = result[0]
        self.create_bucket_obj(bucket, obj)
        self.LOGGER.info("Step 1: Set write permission to object %s", obj)
        res = S3_ACL_OBJ.add_grantee(bucket, obj, canonical_id, permission)
        assert res[0], res[1]
        self.LOGGER.info("Step 1: Permission %s set for the object", obj)
        self.LOGGER.info(
    ...
    def test_3215(self):
        ...
        res = S3_ACL_OBJ.add_grantee(bucket, obj, canonical_id, permission)
        assert res[0], res[1]
        self.LOGGER.info("Step 4: Set write permission to object %s", obj)
        self.LOGGER.info(
    ...
    def test_3216(self):
        ...
        res = S3_ACL_OBJ.add_grantee(bucket, obj, canonical_id, permission)
        assert res[0], res[1]
        self.LOGGER.info("Step 2: Set write permission to object %s", obj)
        self.LOGGER.info(
            "Step 3: Get object acl of object %s after put object acl", obj)
        res = S3_ACL_OBJ.get_object_acl(bucket, obj)
        assert res[0], res[1]
    ...
    def test_3218(self):
        ...
        canonical_id = result[0]
        self.create_bucket_obj(bucket, obj)
        self.LOGGER.info("Step 1: Put permission with invalid XML structure")
        acl = {"Owner": {},
    ...
    def test_3226(self):
        ...
        self.LOGGER.info("Step 1: Completed creating account 1")
        ...
        self.LOGGER.info("Step 2: Completed creating account 2")
        self.create_bucket_obj(bucket, obj)
        self.LOGGER.info(
        ...
        assert res[0], res[1]
        self.LOGGER.info("Step 5: Completed setting permission to "
    ...
    def test_3227(self):
        ...
        canonical_id = result[0]
        self.LOGGER.info("Step 1: Completed creating account")
        self.LOGGER.info("Step 2: Creating bucket %s", bucket)
        res = S3_OBJ.create_bucket(bucket)
        assert res[0], res[1]
        self.LOGGER.info("Step 2: Bucket created")
        self.LOGGER.info("Step 3: Put object acl with invalid object")
        acl = {
            "Owner": {
    ...
    def test_3682(self):
        ...
        test_file_path = OBJ_ACL_CONFIG["common_var"]["file_path"]
        ...
        self.LOGGER.info("Step 1: Creating Bucket and Object")
        ...
        self.LOGGER.info("Step 1: Done Creating Bucket and Object")
        ...
        # User Account Variables
        self.LOGGER.info("Step 3: Creating 2nd Account")
        ...
        self.LOGGER.info("Step 3: Created second account")
        # Creating the 2nd user s3 Object
        try:
            self.LOGGER.info("Step 5: Getting object from user 2")
    ...
    def test_3683(self):
        ...
        result = self.create_s3iamcli_acc(account_name_2, email_id_2)
        # Creating the 2nd user s3-Object
        self.LOGGER.info("Getting object from user 2")
        s3_acl_obj_2 = result[2]
        # Performing the get ACL using the Account 2
        self.LOGGER.info("Getting the Object from user account 2")
        try:
            self.LOGGER.info("Step 5: Getting object from user 2")
            s3_acl_obj_2.get_object_acl(bucket_name, s3obj_name)
        except CTException as error:
    ...
    def test_3684(self):
        ...
        email_id_1 = OBJ_ACL_CONFIG["test_10964"]["emailid"].format(
            self.random_num)
        self.LOGGER.info("Step 1: Creating account 2 ")
        result = self.create_s3iamcli_acc(account_name, email_id_1)
        canonical_id = result[0]
        self.LOGGER.info("Step 1: Completed Creating account 2 completed")
        ...
        assert res[0], res[1]
        assert canonical_id in str(res[1]), res[1]
        self.LOGGER.info("Step 4: Get object acl response is %s", res)
        self.LOGGER.info("Creating s3 ACL object for user 2 account")
    ...
    def test_3685(self):
        ...
        assert resp[0], resp[1]
        # Creating User Account 1
        self.LOGGER.info("Step 1: Creating User Account 1")
        result = self.create_s3iamcli_acc(account_name_1, email_id_1)
        self.LOGGER.info("Step 1: Successfully Created account 1")
        # Creating the new s3 Object
        s3_obj_1 = result[1]
        s3_acl_obj_1 = result[2]
        # Create Bucket with 1st User
        ...
        self.LOGGER.info(
            "Step 2: Done creating Object with canned ACL property")
        self.LOGGER.info("Step 3: Getting Object ACL from the Parent Account")
        obj_acl = s3_acl_obj_1.get_object_acl(bucket_name, s3obj_name)
        assert obj_acl[0], obj_acl[1]
        self.LOGGER.info(
        ...
        # Performing the get ACL using the Account 2
        try:
            self.LOGGER.info("Getting the Object from user second")
            s3_acl_obj_2.get_object_acl(bucket_name, s3obj_name)
        except CTException as error:
    ...
    def test_3686(self):
        ...
        s3_obj_1 = result[1]
        s3_acl_obj_1 = result[2]
        self.LOGGER.info("Step 1: Done creating account 1")
        self.LOGGER.info("Step 2:Create Bucket and object with 1st User")
        buck_resp = s3_obj_1.create_bucket(bucket_name)
        assert buck_resp[0], buck_resp[1]
        # Creating Object inside the bucket
        put_obj_res = s3_obj_1.put_object(
            bucket_name, s3obj_name, test_file_path)
        self.LOGGER.info("Put object resp %s", put_obj_res)
        ...
        self.LOGGER.info("Step 3: Updating the acl property of the object")
        put_acl_res = s3_acl_obj_1.put_object_canned_acl(
        ...
        self.LOGGER.info("Step 4: Verify the acl property of the object")
        obj_acl = s3_acl_obj_1.get_object_acl(bucket_name, s3obj_name)
        assert obj_acl[0], obj_acl[1]
        self.LOGGER.info("Step 4: Verified the acl property of the object")
        # Creating 2nd User Account
        # User Account Variables
        ...
        try:
            self.LOGGER.info("Step 6: Getting the Object from user second")
            s3_acl_obj_2.get_object_acl(bucket_name, s3obj_name)
        except CTException as error:
        ...
        self.LOGGER.info("Object file resp : %s", object_file_resp)
        self.LOGGER.info(
            "Add canned acl authenticated-read for put object acl in account1 "
            "and try to get object from account2")
    ...
    def test_3687(self):
        ...
        self.LOGGER.info("Step 3: Get Object ACL property")
        get_obj_res = S3_ACL_OBJ.get_object_acl(bucket_name, s3obj_name)
        assert get_obj_res[0], get_obj_res[1]
        self.LOGGER.info("Step 4: Get Object ACL resp : %s", get_obj_res)
        # Adding Write Permission for the 2nd User
        self.LOGGER.info(
        ...
        self.LOGGER.info("Getting object from account 2")
        s3_acl_obj_2 = result[2]
        try:
            self.LOGGER.info(
        ...
        try:
            self.LOGGER.info("Get object with get-object-acl from account 2")
            s3_acl_obj_2.get_object_acl(bucket_name, s3obj_name)
        except CTException as error:
            assert error_msg in error.message, error.message
        self.LOGGER.info(
    ...
    def test_3688(self):
        ...
        assert res[0], res[1]
        self.LOGGER.info("Putting object %s to existing bucket %s",
                         s3obj_name, bucket_name)
        res = s3_acl_obj_1.put_object_with_acl(
            bucket_name, s3obj_name, test_file_path, acl=private_acl)
        assert res[0], res[1]
        self.LOGGER.info("Step 2:  Completed creating object and bucket")
        self.LOGGER.info("Step 3: Verifying the Canned Object ACL property")
        obj_acl = s3_acl_obj_1.get_object_acl(bucket_name, s3obj_name)
        assert obj_acl[0], obj_acl[1]
        self.LOGGER.info(
        ...
        email_id_2 = OBJ_ACL_CONFIG["test_10968"]["emailid_2"].format(
            self.random_num)
        self.LOGGER.info("Creating account name %s and email_id %s",
        ...
        self.LOGGER.info(
            "Step 4: Get object acl of object response is", res)
        assert res[0], res[1]
        assert result[0] in str(res[1]), res[0]
    ...
    def test_3689(self):
        ...
        s3_obj_1 = result[1]
        s3_acl_obj_1 = result[2]
        self.LOGGER.info("Step 2: Create Bucket with 1st User")
        buck_resp = s3_obj_1.create_bucket(bucket_name)
        assert buck_resp[0], buck_resp[1]
        self.LOGGER.info("Step 2: Create Bucket with 1st User")
        self.LOGGER.info(
        ...
        self.LOGGER.info("Step 4: Verifying the Canned Object ACL property")
        ...
        # Creating the 2nd user s3 Object
        s3_acl_obj_2 = result[2]
        self.LOGGER.info("Step 5: Adding Read Permission for the 2nd User")
        self.LOGGER.info(
        ...
        self.LOGGER.info(
            ...
            s3obj_name)
        res = s3_acl_obj_1.get_object_acl(bucket_name, s3obj_name)
        self.LOGGER.info("ACP Property : %s", res)
        assert res[0], res[1]
        assert canonical_id_2 in str(res[1]), res[0]
        assert read_acp in str(res[1]), res[0]
        self.LOGGER.info("Step 6: Get object ACL for 2nd User")
        try:
    ...
    def test_3693(self):
        ...
        buck_resp = s3_obj_1.create_bucket(bucket_name)
        assert buck_resp[0], buck_resp[1]
        self.LOGGER.info("Step 2: Successfully Created Bucket with 1st User")
        self.LOGGER.info(
            "Step 3: Creating Object inside the bucket with canned ACL property")
        obj_resp = s3_acl_obj_1.put_object_with_acl(
            bucket_name, s3obj_name, test_file_path, acl=private_acl)
        ...
        self.LOGGER.info("Step 4: Get ACL Object ACL property ")
        obj_acl = s3_acl_obj_1.get_object_acl(bucket_name, s3obj_name)
        assert obj_acl[0], obj_acl[1]
        self.LOGGER.info("Step 4: Get ACL Object resp is %s", obj_acl)
        self.LOGGER.info(
        ...
        try:
            ...
            self.LOGGER.info("Step 6: Object from should return ACCESSIONED")
        self.LOGGER.info(
    ...
    def test_3692(self):
        ...
        # Creating the 2nd user s3 Object
        s3_acl_obj_2 = result[2]
        self.LOGGER.info("Step 6: Getting object ACL from user 2")
        try:
            s3_acl_obj_2.get_object_acl(bucket_name, s3obj_name)
        except CTException as error:
            assert OBJ_ACL_CONFIG["test_10973"]["error_msg"] in error.message, error.message
        self.LOGGER.info("Step 6: Done getting object ACL from user 2")
        self.LOGGER.info(
            "First add private canned ACL to object "
    ...
    def test_3694(self):
        ...
        self.LOGGER.info("Step 2: Successfully Created account 2")
        s3obj_user2 = result[2]
        self.LOGGER.info("Step 3: Create Bucket with 1st User")
        buck_resp = s3_obj_1.create_bucket(bucket_name)
        assert buck_resp[0], buck_resp[1]
        ...
        self.LOGGER.info("GET Object in the bucket using the Account 1")
        try:
    ...
    def test_3695(self):
        ...
        buck_resp = s3_obj_1.create_bucket(bucket_name)
        assert buck_resp[0], buck_resp[1]
        self.LOGGER.info("Step 3: Successfully Created Bucket with 1st User")
        self.LOGGER.info(
        ...
        self.LOGGER.info("Step 6: Check bucket ACL permission")
        resp = s3_acl_obj_1.get_bucket_acl(bucket_name)
        assert resp[0], resp[1]
        assert resp[1][1][0]["Permission"] == write_acp, resp[1]
        self.LOGGER.info("Step 6: bucket ACL permission resp is %s", resp)
    ...
    def test_3504(self):
        ...
        self.LOGGER.info("Step 2: Bucket was created")
        self.LOGGER.info(
    ...
    def test_3509(self):
        ...
        self.LOGGER.info(
            ...
            account_name, email)
        result = self.create_s3iamcli_acc(account_name, email)
        self.LOGGER.info("Step 1: Successfully created the account")
        self.LOGGER.info(
            "Step 2 :Creating bucket using %s account credentials",
            account_name)
        s3_obj_1 = result[1]
        ...
        assert resp[0], resp[1]
        self.LOGGER.info("Step 2: Successfully created the bucket")
        self.LOGGER.info("Step 3: Putting object with acl")
        try:
            s3_acl_obj_1.put_object_with_acl(
                bucket_name,
                key,
        ...
        self.LOGGER.info("Step 3: Done putting object with acl and grants")
        self.LOGGER.info(
            "Add canned ACL bucket-owner-read along with READ ACL grant permission")
    ...
    def test_3543(self):
        ...
        self.LOGGER.info("Step 3: Putting object with invalid acl permission")
        try:
            s3_acl_obj_1.put_object_with_acl(
                bucket_name,
                key,
    ...
    def test_3551(self):
        ...
        self.LOGGER.info(
            ...
            account_name, email)
        result = self.create_s3iamcli_acc(account_name, email)
        self.LOGGER.info("Step 1: Account was Created Successfully")
        self.LOGGER.info(
            "Step 2: Creating bucket using %s account credentials",
            account_name)
        s3_obj_1 = result[1]
    ...
    def test_3496(self):
        ...
        resp = create_file(
            ...
            OBJ_ACL_CONFIG["common_var"]["file_size"])
        assert resp[0], resp[1]
        self.LOGGER.info("Step 1: Creating bucket")
        resp = S3_OBJ.create_bucket(bucket_name)
        assert resp[0], resp[1]
        self.LOGGER.info("Step 1: Successfully bucket was created")
        self.LOGGER.info("Step 2: Putting object with acl permission")
        ...
        self.LOGGER.info("Step 3: Getting the object acl")
        res = S3_ACL_OBJ.get_object_acl(bucket_name, obj)
        assert res[0], res[1]
        assert OBJ_ACL_CONFIG["test_10712"]["assert_msg"] in str(
            res[1]), res[0]
        self.LOGGER.info("Step 3: Object was uploaded with ACL permission")
        self.LOGGER.info(
            "Verify bucket-owner-read canned ACL when object belongs to the bucket owner")
    ...
    def test_3497(self):
        ...
        assert OBJ_ACL_CONFIG["test_10712"]["assert_msg"] in str(
            res[1]), res[0]
        self.LOGGER.info("Step 3: Object response was verified")
        self.LOGGER.info(
            "Verify bucket-owner-full-control canned ACL when object belongs to the bucket owner")
    ...
    def test_3498(self):
        ...
        self.LOGGER.info("step 1: Creating User Account 1 and 2")
        ...
        s3obj_user2 = result[2]
        self.LOGGER.info("Step 1: Done creating account 1 and 2")
        self.LOGGER.info("Step 2 : Creating Bucket")
        resp = s3_obj_1.create_bucket(bucket_name)
        assert resp[0], resp[1]
        self.LOGGER.info("Step 2 : Bucket was created")
        self.LOGGER.info("Step 3 : Put bucket with grant write permission")
        ...
        self.LOGGER.info("Step 3: Check bucket ACL permission")
        resp = s3_acl_obj_1.get_bucket_acl(bucket_name)
        assert resp[0], resp[1]
        assert resp[1][1][0]["Permission"] == OBJ_ACL_CONFIG["test_10714"]["write_assert"], resp[1]
        self.LOGGER.info("Step 3: Bucket ACL verified")
        self.LOGGER.info(
    ...
    def test_3499(self):
        ...
        canonical_id_user_2 = result[0]
        s3obj_user2 = result[2]
        self.LOGGER.info("Step 1: Completed Creating User Accounts 1 and 2")
        self.LOGGER.info("Step 2: Creating Bucket")
        ...
        self.LOGGER.info("Step 3: Bucket permission was verified")
        self.LOGGER.info(
    ...
    def test_3502(self):
        ...
        s3obj_user = result[1]
        s3acl_user = result[2]
        self.LOGGER.info("Step 1: Completed Creating User Accounts 1")
        self.LOGGER.info("Step 2: Create Bucket")
        resp = s3obj_user.create_bucket(bucket_name)
        assert resp[0], resp[1]
        self.LOGGER.info("Step 3: Bucket was created")
        self.LOGGER.info("Step 4: Upload object")
        ...
        self.LOGGER.info("Step 5: Get object ACL")
        resp = s3acl_user.get_object_acl(bucket_name, key)
        assert resp[0], resp[1]
        self.LOGGER.info("Step 5: Get object ACL resp is verified")
        self.LOGGER.info(
        ...
        self.LOGGER.info("Step 7: Get object ACL after overwrite")
        resp = s3acl_user.get_object_acl(bucket_name, key)
        assert resp[0], resp[1]
        assert resp[1]["Grants"][0]["Permission"] == \
            OBJ_ACL_CONFIG["test_10718"]["full_cntrl"], resp[1]
        self.LOGGER.info("Step 7: Get object ACL response is verified")
        self.LOGGER.info(
            "First add bucket-owner-read canned ACL to object and after that overwrite"
            "bucket-owner-full-control canned ACL to same object")
    ...
    def test_3503(self):
        ...
        self.LOGGER.info("Step 3: Get object ACL")
        resp = s3acl_user.get_object_acl(bucket_name, key)
        assert resp[0], resp[1]
        self.LOGGER.info("Step 3: Get object ACL response is verified")
        self.LOGGER.info("Step 4: Put object and get ACL ")
        resp = s3acl_user.put_object_canned_acl(
    ...
    def test_3505(self):
        ...
        self.LOGGER.info(
            ...
            account_name, email)
        result = self.create_s3iamcli_acc(account_name, email)
        self.LOGGER.info("Step 1: Account was created")
        acl_obj = result[2]
        s3obj_user = result[1]
        self.LOGGER.info(
            "Step 2:Creating bucket using %s account credentials",
        ...
        resp = s3obj_user.create_bucket(bucket_name)
        assert resp[0], resp[1]
        self.LOGGER.info("Step 2:Bucket was created successfully")
    ...
    def test_3506(self):
        ...
        resp = s3obj_user.create_bucket(bucket_name)
        assert resp[0], resp[1]
        self.LOGGER.info("Step 2: Bucket was successfully created")
        self.LOGGER.info(
    ...
    def test_3507(self):
        ...
        assert resp[0], resp[1]
        self.LOGGER.info("Step 2: Bucket was created successfully")
        self.LOGGER.info("Step 3: Putting object with acl permission")
    ...
    def test_3508(self):
        ...
        acl_obj = result[2]
        s3obj_user = result[1]
        self.LOGGER.info("Step 1: Successfully account was created")
        self.LOGGER.info(
            "Step 2: Creating bucket using %s account credentials",
            account_name)
        resp = s3obj_user.create_bucket(bucket_name)
    ...
    def test_3510(self):
        ...
        put_res = s3obj_user.put_object(bucket_name, key, test_file_path)
        assert put_res[0]
        self.LOGGER.info("Step 3: Putting object with canned acl")
    ...
    def test_3553(self):
        ...
        self.LOGGER.info("Step 1: Put Object")
        resp = s3obj_user.put_object(bucket_name, obj, test_file_path)
        assert resp[0], resp[1]
        self.LOGGER.info(
    ...
    def test_3554(self):
        ...
        assert res[0], res[1]
        self.LOGGER.info("Step 3: Object was uploaded")
        self.LOGGER.info("Step 4: Put full control and private ACL to object")
        res = S3_ACL_OBJ.add_grantee(
            bucket,
            obj,
            canonical_id,
    ...
    def test_159(self):
        ...
        result = self.create_s3iamcli_acc(account_name_2, email_id_2)
        s3obj_user2 = result[2]
        self.LOGGER.info("Step 1: Successfully created account 1 and 2")
        self.LOGGER.info("Step 2: Create Bucket with Account 1")
        buck_resp = s3obj_user.create_bucket(bucket_name)
        self.LOGGER.info("Create Bucket response is : %s", buck_resp)
        assert buck_resp[0], buck_resp[1]
        self.LOGGER.info("Step 2: Bucket was created successfully")
        self.LOGGER.info("Step 3: Put Object using Account 1")
    ...
    def test_170(self):
        ...
        result = self.create_s3iamcli_acc(account_name_2, email_id_2)
        s3obj_user2 = result[2]
        self.LOGGER.info("Step 1: Accounts was created successfully")
        buck_resp = s3obj_user.create_bucket(bucket_name)
        self.LOGGER.info(
    ...
    def test_172(self):
        ...
        result = self.create_s3iamcli_acc(account_name_2, email_id_2)
        s3obj_user2 = result[2]
        self.LOGGER.info("Step 1: Account were created successfully")
        self.LOGGER.info("Step 2: Create Bucket with Account 1")
        buck_resp = acl_obj.create_bucket_with_acl(
            bucket_name, grant_write="{}{}".format(
                OBJ_ACL_CONFIG["test_172"]["id_str"], result[0]))
        self.LOGGER.info("Bucket resp is %s", buck_resp)
        assert buck_resp[0], buck_resp[1]
        self.LOGGER.info("Step 2: Bucket was created")
        ...
        self.LOGGER.info("Step 4: Get object acl resp was verified")
        self.LOGGER.info(
        ...
        resp = s3obj_user2.get_object_acl(bucket_name, s3obj_name)
        assert resp[1]["Grants"][0]["Permission"] == usr2_acl, resp[1]
        self.LOGGER.info("Step 6: Get object ACL resp was verified")
        self.LOGGER.info(
        ...
        put_acl_res = s3obj_user2.put_object_canned_acl(
            bucket_name, s3obj_name, acl=can_object_acl)
        assert put_acl_res[0]
        self.LOGGER.info("Step 6: ACL property was updated successfully")
        self.LOGGER.info("Step 7: Verifying the object acl using Account 1 "
    ...
    def test_175(self):
        ...
        can_id_2 = result[0]
        s3obj_user2 = result[2]
        self.LOGGER.info("Step 1 : Successfully accounts were created")
        self.LOGGER.info(
        ...
        put_acl_res = s3obj_user2.put_object_with_acl(
            bucket_name, s3obj_name, test_file_path, acl=can_object_acl)
        assert put_acl_res[0]
        self.LOGGER.info("Step 3: Object was uploaded in the bucket")
        self.LOGGER.info(
            "Step 4: Get object acl of object %s after put object acl",
            s3obj_name)
        ...
        res = acl_obj.get_object_acl(bucket_name, s3obj_name)
        assert res[0], res[1]
        self.LOGGER.info("Step 6: Object ACL was verified")
        self.LOGGER.info(
    ...
    def test_453(self):
        ...
        # Creating User Account 1
        result = self.create_s3iamcli_acc(account_name_1, email_id_1)
        self.LOGGER.info("Successfully Created account 1")
        # Creating the new s3 Object
        s3_obj_1 = result[1]
        s3_acl_obj_1 = result[2]
        ...
        buck_resp = s3_obj_1.create_bucket(bucket_name)
        assert buck_resp[0], buck_resp[1]
        create_file(OBJ_ACL_CONFIG["common_var"]["file_path"],
                    OBJ_ACL_CONFIG["common_var"]["file_size"])
        self.LOGGER.info(
        ...
        self.LOGGER.info("Step 5 : Switch to Account 2")
    ...
    def test_423(self):
        ...
        self.LOGGER.info("Step 5 : Switch to Account 2")
        self.LOGGER.info("Step 5 : Done in STEP 3 Switch to Account 2")
    ...
    def test_421(self):
        ...
        self.LOGGER.info("Step 3: Apply grant WRITE permission to account2")
    ...
    def test_419(self):
        ...
        self.LOGGER.info("Step 3: Apply grant READ permission to account2")
    ...
    def test_410(self):
        ...
        s3obj_name = OBJ_ACL_CONFIG["common_var"]["object_name"].format(
            self.random_num)
        self.LOGGER.info("Step 1: Create tags on existing object")
        self.LOGGER.info(
        ...
        assert resp[0], resp[1]
        self.LOGGER.info("Step 1: Done Create tags on existing object")
    ...
    def test_286(self):
        ...
        self.LOGGER.info("Step 2: Get object acl")
        res = S3_ACL_OBJ.get_object_acl(bucket, obj)
        assert res[0], res[1]

        self.LOGGER.info("Step 3: Get object acl from account 2")
    ...
    def test_3453(self):
        ...
        self.LOGGER.info("Step 1: Creating two accounts...")
        ...
        self.LOGGER.info("Step 1: Created two accounts successfully")
        self.create_bucket_obj(bucket_name, s3obj_name, s3_test_obj1)
        self.LOGGER.info("Step 4: Creating a custom acl xml")
        ...
        self.LOGGER.info(modified_acp)
        self.LOGGER.info("Step 4: Created a custom acl xml")
        self.LOGGER.info(
        ...
        self.LOGGER.info(
            ...
            obj_permission)
        self.LOGGER.info("Step 6: Retrieving an object using second account")
    ...
    def test_3455(self):
        ...
        self.LOGGER.info("Step 1: Creating three accounts...")
        ...
        self.LOGGER.info("Step 1: Created three accounts successfully")
        self.create_bucket_obj(bucket_name, s3obj_name, s3_test_obj1)
        self.LOGGER.info("Step 2: Creating a custom acl xml")
        resp = s3_acl_obj1.get_object_acl(bucket_name, s3obj_name)
        assert resp[0], resp[1]
        grantee_json["Grantee"]["DisplayName"] = grantee_json["Grantee"]["DisplayName"].format(
            account_name_2)
        ...
        modified_acp["Grants"][0] = new_grantee
        self.LOGGER.info(modified_acp)
        self.LOGGER.info("Step 2: Created a custom acl xml")
        self.LOGGER.info(
        ...
        self.LOGGER.info(
            ...
            obj_permission)
        self.LOGGER.info("Step 4: Retrieving an object using third account")
        try:
    ...
    def test_3456(self):
        ...
        self.LOGGER.info(
            "Step 3: Set a %s permission to an object for second account",
            obj_permission)
        self.LOGGER.info("Step 4: Retrieving an object using second account")
        try:
    ...
    def test_3228(self):
        ...
        obj_name = OBJ_ACL_CONFIG["common_var"]["object_name"].format(
            self.random_num)
        self.LOGGER.info("Bucket and Object : %s %s", bkt_name, obj_name)
        self.create_bucket_obj(bkt_name, obj_name)
        obj_acl_json = OBJ_ACL_CONFIG["test_3228"]["policy"]
        try:
            self.LOGGER.info("Step 1: Put invalid JSON")
    ...
    def test_3250(self):
        ...
        s3_obj_2 = result[1]
        self.create_bucket_obj(bkt_name, obj_name, S3_OBJ)
        self.LOGGER.info("Step 2: Get object using account 2")
        try:
    ...
    def test_3254(self):
        ...
        try:
            ...
            self.LOGGER.info(
                "Step 2: get object using acc 2 failed with err message: %s",
                error.message)
        self.LOGGER.info("Step 2: Get object using account 2 was successful")
        self.LOGGER.info(
    ...
    def test_3255(self):
        ...
        s3_obj_2 = self.create_acc_and_put_obj_acp(
            bkt_name, obj_name, test_cfg)
        self.LOGGER.info("Step 1: Get object using account 2")
        resp = s3_obj_2.get_object(bkt_name, obj_name)
        assert resp[0], resp[1]
        self.LOGGER.info("Step 1: Get object using account 2 was successful")
        self.LOGGER.info(
