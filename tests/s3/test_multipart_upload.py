...
class TestMultipartUpload:
    ...
    def setup_class(cls):
        ...
        cls.aws_config_path = []
        cls.aws_config_path.append(MPART_CFG["multipart"]["aws_config_path"])
    ...
    def setup_method(self):
        ...
        self.log.info(
            "Taking a backup of aws config file located at {0} to {1}...".format(
        ...
        self.log.info(
            "Taken a backup of aws config file located at {0} to {1}".format(
                self.aws_config_path, self.backup_path))
    ...
    def teardown_method(self):
        ...
        self.log.info(
            "Restoring aws config file from {0} to {1}...".format(
                self.backup_path,
        ...
        assert_true(resp[0], resp[1])
        self.log.info(
            "Restored aws config file from {0} to {1}".format(
                self.backup_path,
                self.aws_config_path))
    ...
    def create_bucket_to_upload_parts(
        ...
        assert_true(res[0], res[1])
        assert_equal(len(res[1]), total_parts, res[1])
        parts = res[1]
        self.log.info("Uploaded parts into bucket: {0}".format(parts))
        return mpu_id, parts
    ...
    def test_2061_2065_2066_2069(self):
        ...
        res = S3_MP_TEST_OBJ.list_parts(
            ...
            mp_config["object_name"])
        assert_true(res[0], res[1])
        assert_equal(len(res[1]["Parts"]),
                     mp_config["total_parts"], res[1])
        self.log.info("Listed parts of multipart upload: {0}".format(res[1]))
        ...
        assert_in(mp_config["object_name"], res[1], res[1])
    ...
    def test_2070(self):
        ...
        res = S3_MP_TEST_OBJ.list_multipart_uploads(
            mp_config["bucket_name"])
        assert_not_in(mpu_id, res[1], res[1])
        self.log.info(
            "Aborted multipart upload with upload ID: {0}".format(mpu_id))
        self.log.info("Abort Multipart upload")
    ...
    def test_2062(self):
        ...
        self.log.info(
            "Creating a bucket with name : {}".format(
                mp_config["bucket_name"]))
        res = S3_TEST_OBJ.create_bucket(mp_config["bucket_name"])
        assert_true(res[0], res[1])
        assert_equal(res[1], mp_config["bucket_name"], res[1])
        self.log.info(
            "Created a bucket with name : {}".format(mp_config["bucket_name"]))
        sp_file = split_file(
            self.mp_obj_path,
            mp_config["file_size"],
            mp_config["total_parts"],
    ...
    def test_2068(self):
        ...
        assert_in(mpu_id, str(res[1]), res[1])
        self.log.info(
            "Listed multipart uploads: {0}".format(
                res[1]["Uploads"]))
        self.log.info("List Multipart uploads")
    ...
    def test_2071(self):
        ...
        for cfg, value in mp_s3_config_list:
            ...
            assert_in(value, str(res))
        self.log.info("Applied aws s3 configurations for multipart upload")
        self.log.info(
            "Creating a bucket with name : {0}".format(
                mp_config["bucket_name"]))
        res = S3_TEST_OBJ.create_bucket(mp_config["bucket_name"])
        assert_true(res[0], res[1])
        assert_equal(res[1], mp_config["bucket_name"], res[1])
        self.log.info(
            "Created a bucket with name : {0}".format(
                mp_config["bucket_name"]))
        self.log.info("Creating and uploading a file:{0} ".format(
            self.mp_obj_path))
        res = create_file(
            self.mp_obj_path,
            mp_config["file_size"])
        ...
        assert_true(res[0], res[1])
        self.log.info(
            "Uploaded an object{0} to the bucket{1}".format(
                mp_config["object_name"],
                mp_config["bucket_name"]))
    ...
    def test_2296(self):
        ...
        try:
            ...
            assert_in(
                ...
                error.message)
        res = S3_TEST_OBJ.object_list(mp_config["bucket_name"])
        assert_not_in(mp_config["object_name"], res[1], res[1])
    ...
    def test_2298(self):
        ...
        for count in range(mp_config["total_mp_uploads"]):
            ...
            assert_equal(len(res[1]["Parts"]),
                         mp_config["total_parts"], res[1])
            self.log.info(
                "Listed parts of multipart upload: {0}".format(
                    res[1]))
    ...
    def test_2300(self):
        ...
        res = run_local_cmd(
            "{0} {1}".format(
                aws_get_cmd,
                mp_config["s3_configs"]))
        assert_in(mp_config["max_concurrent_requests"], str(res))
        ...
        res = run_local_cmd(
            "{0} {1}".format(
                aws_get_cmd,
                mp_config["s3_configs"]))
        assert_in(mp_config["default_max_concurrent_requests"], str(res))
    ...
    def test_2301(self):
        ...
        res = run_local_cmd(
            "{0} {1}".format(
                aws_get_cmd,
                mp_config["s3_configs"]))
        assert_in(mp_config["multipart_threshold"], str(res))
        ...
        res = run_local_cmd(
            "{0} {1}".format(
                aws_get_cmd,
                mp_config["s3_configs"]))
        assert_in(mp_config["default_multipart_threshold"], str(res), res)
    ...
    def test_631(self):
        ...
        self.log.info(
            "Step 1: Creating a bucket with name : {}".format(
                mp_config["bucket_name"]))
        res = S3_TEST_OBJ.create_bucket(mp_config["bucket_name"])
        assert_true(res[0], res[1])
        assert_equal(res[1], mp_config["bucket_name"], res[1])
        self.log.info(
            "Step 1: Created a bucket with name : {}".format(
                mp_config["bucket_name"]))
        ...
        mpu_id = res[1]["UploadId"]
        self.log.info(
            "Step 2: Multipart Upload initiated with mpu_id {0}".format(mpu_id))
        self.log.info(
            "Step 3: Create wrong json for input as multipart-upload")
        wrong_json = mp_config["wrong_json"]
        self.log.info(
            f"Step 3: Created wrong json for input as multipart-upload {wrong_json}")
        self.log.info(
