...
class TestUserLoginProfileTests():
    ...
    def delete_accounts_and_users(self):
        ...
        self.log.debug("IAM users: {0}".format(iam_users_list))
        if iam_users_list:
            ...
            for user in iam_users_list:
                ...
                self.log.debug("Deleted user : {0}".format(user))
        ...
        self.log.debug("IAM accounts: {0}".format(iam_accounts))
        if iam_accounts:
            self.log.debug("Deleting IAM accounts...")
            for acc in iam_accounts:
                self.log.debug("Deleting {0} account".format(acc))
    ...
    def create_user_and_access_key(
        ...
        """
        self.log.info("Creating a user with name {0}".format(user_name))
        resp = IAM_TEST_OBJ.create_user(user_name)
        assert_true(resp[0], resp[1])
        self.log.info("Created a user with name {0}".format(user_name))
        self.log.info("Creating login profile for user {0}".format(user_name))
        ...
        self.log.info("Creating access key for user {0}".format(user_name))
    ...
    def create_account_and_user(
        ...
        """
        user_cfg = USER_CONFIG["iam_user_login"]
        self.log.info("Creating account with name {0}".format(acc_name))
        resp = IAM_TEST_OBJ.create_account_s3iamcli(
            acc_name, email_id, ldap_user, ldap_pwd)
        assert_true(resp[0], resp[1])
        self.log.info("Created account with name {0}".format(acc_name))
        ...
        if user_profile:
            self.log.info(
                "Creating user login profile for user {0}".format(user_name))
            resp = IAM_TEST_OBJ.create_user_login_profile_s3iamcli(
                user_name, user_password, pwd_reset, access_key, secret_key)
            assert_true(resp[0], resp[1])
            self.log.info(
                "Created user login profile for user {0}".format(user_name))
        return access_key, secret_key
    ...
    def test_2846(self):
        ...
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "Verify update-login-profile (password change) for IAM user")
    ...
    def test_2848(self):
        """
        Verify update-login-profile (password change) for IAM user with 'Blank' or 'NO' password
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "Verify update-login-profile (password change) for IAM user with 'Blank' or 'NO' password")
    ...
    def test_2854(self):
        """
        verify update-login-profile with password having combinations of special characters  _+=,.@-
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "verify update-login-profile with password having combinations of special characters  _+=,.@-")
    ...
    def test_2855(self):
        """
        Update login profile for IAM user without mentioning  --password-reset-required --no-password-reset-required
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "Update login profile for IAM user without mentioning  --password-reset-required --no-password-reset-required")
    ...
    def test_2856(self):
        """
        update login profile for IAM user with both options --no-password-reset-required --password-reset-required
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "update login profile for IAM user with both options --no-password-reset-required --password-reset-required")
    ...
    def test_2857(self):
        ...
        try:
            IAM_TEST_OBJ.update_user_login_profile_without_passowrd_and_reset_option(
                test_9835_cfg["user_name"], S3_HLPR.get_local_keys()[0], S3_HLPR.get_local_keys()[1])
        except CTException as error:
            self.log.debug(error.message)
            assert_in(
    ...
    def test_2858(self):
        ...
        :avocado: tags=user_login_profile
        """
        self.log.info("Create a login profile for the existing IAM user")
    ...
    def test_2859(self):
        ...
        :avocado: tags=user_login_profile
        """
        self.log.info("Create a login profile for the non-existing IAM user")
    ...
    def test_2860(self):
        """
        Create a login profile with password of 0 character or without password for existing user
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "Create a login profile with password of 0 character or without password for existing user")
    ...
    def test_2865(self):
        """
        Create a login profile with password - try few combinations of special characters and alphanumberic characters
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "Create a login profile with password - try few combinations of special characters and alphanumberic characters")
        ...
        for password in test_9843_cfg["special_char_pwd"]:
            ...
            assert_true(resp[0], resp[1])
            self.log.debug(
                "Creating user login profile with password: {0}".format(password))
            resp = IAM_TEST_OBJ.create_user_login_profile(
    ...
    def test_2866(self):
        ...
        self.log.debug(
            "Creating {0} users".format(
                test_9844_cfg["no_of_users"]))
        for n in range(test_9844_cfg["no_of_users"]):
            ...
            self.log.debug(
                "Creating a user with name: {0}".format(new_user_name))
    ...
    def test_2869(self):
        """
        Create login profile for IAM user without mentioning  --password-reset-required --no-password-reset-required .
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "Create login profile for IAM user without mentioning  --password-reset-required --no-password-reset-required .")
    ...
    def test_2870(self):
        """
        Create login profile for IAM user with both options --no-password-reset-required --password-reset-required .
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "Create login profile for IAM user with both options --no-password-reset-required --password-reset-required .")
    ...
    def test_2871(self):
        ...
        :avocado: tags=user_login_profile
        """
        self.log.info("Verify get-login-profile for s3 IAM user")
    ...
    def test_2872(self):
        ...
        :avocado: tags=user_login_profile
        """
        self.log.info("Verify get-login-profile for non-existing s3 IAM user")
    ...
    def test_2873(self):
        """
        Verify get-login-profile for a non profile IAM user (IAM user with no profile created)
        :avocado: tags=user_login_profile
        """
        self.log.info(
            "Verify get-login-profile for a non profile IAM user (IAM user with no profile created)")
    ...
    def test_2897(self):
        ...
        :avocado: tags=change_password
        """
        self.log.info("Verify password change for IAM user")
    ...
    def test_2898(self):
        ...
        :avocado: tags=change_password
        """
        self.log.info("Verify password change for a non-existing IAM user")
    ...
    def test_2899(self):
        ...
        :avocado: tags=change_password
        """
        self.log.info("Provide only six character length in password")
    ...
    def test_2849(self):
        ...
        :avocado: tags=change_password
        """
        self.log.info("Provide only one character length in password")
    ...
    # @ctp_fail_on(error_handler) #Duplicate of test_2850
    # def test_9880(self):
    #     """
    #     Provide password length 128 valid characters long
    #     :avocado: tags=change_password
    ...
    def test_2903(self):
        ...
        -, _, +, =, \, /, ?, .., \n, \t, \rFor special characters &, <, > and | , if user want to use these then
    ...
    def test_2904(self):
        ...
        :avocado: tags=change_password
        """
        self.log.info("Verify change password with old password")
    ...
    def test_2905(self):
        """
        Verify change password for the user with users invalid access key and secret key
        :avocado: tags=change_password
        """
        self.log.info(
            "Verify change password for the user with users invalid access key and secret key")
    ...
    def test_2929(self):
        ...
        :avocado: tags=get_user_temp_creds
        """
        self.log.info("Get temporary credentials for valid user")
        user_cfg = USER_CONFIG["iam_user_login"]
        ...
        self.log.info(
            "Creating account with name {0} and email id {1}".format(
                test_9923_cfg["account_name"],
                email_id))
        resp = IAM_TEST_OBJ.create_account_s3iamcli(
            test_9923_cfg["account_name"],
        ...
        secret_key = resp[1]["secret_key"]
        self.log.info(
            "Creating account login profile for account {0}".format(
        ...
        assert_true(resp[0], resp[1])
        self.log.info(
            "Creating user {0} for account {1}".format(
        ...
        assert_true(resp[0], resp[1])
        self.log.info(
            "Creating user login profile for user {0}".format(
                test_9923_cfg["user_name"]))
        resp = IAM_TEST_OBJ.create_user_login_profile_s3iamcli(
            test_9923_cfg["user_name"],
        ...
        assert_true(resp[0], resp[1])
        self.log.info(
            "Getting temporary credentials for user {0}".format(
    ...
    def test_2931(self):
        ...
        self.log.info("Creating account with name {0} and email id {1}".format(
        ...
        self.log.info(
            "Creating user with name {0}".format(
        ...
        self.log.info(
            "Deleting user {0}".format(
                test_9925_cfg["user_name"]))
        res = new_iam_obj.delete_user(test_9925_cfg["user_name"])
        assert_true(res[0], res[1])
        self.log.info(
            "Getting temporary credentials for recently deleted user")
        try:
            IAM_TEST_OBJ.get_temp_auth_credentials_user(
                test_9925_cfg["account_name"],
                test_9925_cfg["user_name"],
    ...
    def test_2932(self):
        ...
        assert_true(res[0], res[1])
        self.log.info(
            "Performing s3 operations using users temporary credentials")
    ...
    def test_2934(self):
        """
        Get temporary credentials for the user which doesn't contain the user login profile for that user
        :avocado: tags=get_user_temp_creds
        """
        self.log.info(
            "Verify and perform s3 operations by using user invalid temporary credentials")
        ...
        self.create_account_and_user(
            ...
            user_profile=False)
        self.log.info(
            "Getting temporary credentials for user {0} which "
            "does not contain login profile".format(
                test_29_cfg["user_name"]))
        try:
            IAM_TEST_OBJ.get_temp_auth_credentials_user(
                test_29_cfg["account_name"],
    ...
    def test_2936(self):
        ...
        self.create_account_and_user(
            ...
            user_profile=True)
        self.log.info(
            "Getting temporary credentials for user {0} with 20 min time duration".format(
                test_31_cfg["user_name"]))
        res = IAM_TEST_OBJ.get_temp_auth_credentials_user(
            test_31_cfg["account_name"],
            test_31_cfg["user_name"],
    ...
    def test_2937(self):
        """
        Verify time duration less than 15 mins for the Get temporary credentails for the valid user
        :avocado: tags=get_user_temp_creds
        """
        self.log.info(
            "Verify time duration less than 15 mins for "
        ...
        self.create_account_and_user(
            ...
            user_profile=True)
        self.log.info(
            "Getting temp auth credentials for user {0} "
            "with less than 20 min duration".format(test_32_cfg["user_name"]))
        try:
            IAM_TEST_OBJ.get_temp_auth_credentials_user(
                test_32_cfg["account_name"],
                test_32_cfg["user_name"],
            ...
            assert_in(
                ...
                error.message)
        self.log.info(
            "Verify time duration less than 15 mins for the "
            "Get temporary credentails for the valid user")

    def test_2939(self):
        """
        List users By using Users get temporay credentials.
        :avocado: tags=get_user_temp_creds
        """
        self.log.info(
            "List users By using Users get temporay credentials")
        ...
        self.create_account_and_user(
            ...
            user_profile=True)
        self.log.info(
            "Getting temp auth credentials for user {0}".format(
                test_36_cfg["user_name"]))
        res = IAM_TEST_OBJ.get_temp_auth_credentials_user(
            test_36_cfg["account_name"],
            test_36_cfg["user_name"],
        ...
        try:
            ...
            assert_in(
                ...
                error.message,
                error.message)
        self.log.info("List users By using Users get temporay credentials.")
