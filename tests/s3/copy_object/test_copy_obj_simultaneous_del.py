...
class TestCopyObjectSimultaneousDelete:
    ...
    def create_put_parallel_copy_and_delete_object(self, copy_args, delete_args, is_mpu=False):
        ...
        for obj_size in [1024, 5*1024]:
            ...
            if copy_args[0] == copy_args[2] and copy_args[1] == copy_args[3]:
                self.head_object_wrapper(copy_args[0], copy_args[1], errmsg.NOT_FOUND_ERRCODE)
    ...
    def create_put_parallel_copy_and_delete_bucket(self, copy_args, delete_args, is_mpu=False):
        ...
        for obj_size in [1024, 5*1024]:
            ...
            if is_mpu:
                ...
                self.create_put_object(copy_args[0], copy_args[1], obj_size)
            LOGGER.info("Parallely, \n1. From %s copy %s to %s as %s\n2. Delete %s",
                        copy_args[0], copy_args[1], copy_args[2], copy_args[3], delete_args[0])
            with multiprocessing.Pool(processes=2) as pool:
                process1 = pool.apply_async(self.copy_object_wrapper, args=copy_args[:5])
                process2 = pool.apply_async(self.delete_bucket_wrapper, args=delete_args)
                assert_utils.assert_true(process1.get()[0], process1.get()[1])
                assert_utils.assert_true(process2.get()[0], process2.get()[1])
                resp = self.s3_obj.bucket_list()
                assert_utils.assert_true(resp[0], resp[1])
                if delete_args[0] not in resp[1]:
                    if delete_args[0] in self.buckets:
                        self.buckets.remove(delete_args[0])
                    destbuck = "dest-buck-{}".format(perf_counter_ns())
                    resp = self.s3_obj.create_bucket(destbuck)
                    assert_utils.assert_true(resp[0], resp[1])
                    delete_args_lst = list(delete_args)
                    delete_args_lst[0] = destbuck
                    delete_args = tuple(delete_args_lst)
                    copy_args_lst = list(copy_args)
                    copy_args_lst[2] = destbuck
                    copy_args = tuple(copy_args_lst)
    ...
    def test_45527(self):
        ...
        self.create_put_parallel_copy_and_delete_object((self.srcbuck, "obj1", self.srcbuck,
                                                         ...
                                                         self.file_path), (self.srcbuck, "obj1"),
                                                        is_mpu=True)
    ...
    def test_45528(self):
        ...
        LOGGER.info("STARTED: Test Delete destination bucket when copy multipart object is in "
                    "progress")
        self.create_put_parallel_copy_and_delete_bucket((self.srcbuck, "src-obj", self.destbuck,
                                                         "dest-obj", errmsg.NO_BUCKET_OBJ_ERR_KEY, 5,
