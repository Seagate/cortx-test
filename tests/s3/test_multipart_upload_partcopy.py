...
class TestMultipartUploadPartCopy:
    ...
    def multiprocess_upload_parts(self, mpu_id, parts):
        ...
        all_parts.append({"PartNumber": 2, "ETag": response[1]["ETag"]})
    ...
    def test_mpu_upload_partcopy_32702(self):
        ...
        response = self.s3mpu_obj.upload_part_copy(self.bucket_name + "/" + self.object_name,
                                                   ...
                                                   part_number=2,
                                                   upload_id=mpu_id2)
        new_parts.append({"PartNumber": 2, "ETag": response[1]["ETag"]})
        self.log.info("Listing parts of multipart upload")
        res = self.s3mpu_obj.list_parts(mpu_id2, self.mpu_partcopy_bkt, self.mpu_partcopy_obj)
        assert_utils.assert_true(res[0], res[1])
        self.log.info("Complete the multipart")
        resp = self.s3mpu_obj.complete_multipart_upload(mpu_id2, new_parts[1],
                                                        self.mpu_partcopy_bkt,
                                                        self.mpu_partcopy_obj)
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_mpu_upload_partcopy_32714(self):
        ...
        uploaded_parts2.pop(2)  # removed part 2 as we are going to upload only one part here
        self.log.info("Uploading parts")
        self.multiprocess_upload_parts(mpu_id2, uploaded_parts2)
        self.log.info("Listing parts of multipart upload")
        res = self.s3mpu_obj.list_parts(mpu_id2, self.mpu_partcopy_bkt, self.mpu_partcopy_obj)
        assert_utils.assert_true(res[0], res[1])
        self.log.info("Abort multipart upload")
    ...
    def test_32721(self):
        ...
        resp = self.s3mpu_obj.create_multipart_upload(self.bucket_name, self.object_name)
        assert_utils.assert_true(resp[0], resp[1])
        ...
        resp = self.s3mpu_obj.list_parts(mpu_id, self.bucket_name, self.object_name)
        assert_utils.assert_true(resp[0], resp[1])
        ...
        self.log.info("sorted_part_list: %s", sorted_part_list)
        resp = self.s3mpu_obj.complete_multipart_upload(
    ...
    def test_32730(self):
        ...
        self.log.info("Step 3: Upload 10 parts of various sizes and in random order. The uploaded "
                      "parts can have sizes aligned and unaligned with motr unit sizes."
                      " Make sure the entire object is <= 5GB")
        resp = create_file(self.test_file, count=7, b_size="533M")
        assert_utils.assert_true(resp[0], resp[1])
    ...
    def test_32734(self):
        ...
        self.log.info("Start S3 background IOs.")
        self.s3bio_obj.start(log_prefix="TEST-32734_s3bench_ios", duration="0h5m")
        self.log.info("Step 1: Create bucket1.")
        resp = self.s3t_obj.create_bucket(self.bucket_name)
        assert_utils.assert_true(resp[0], resp)
        self.log.info("Step 2: Initiate multipart upload by performing CreateMultipartUpload.")
