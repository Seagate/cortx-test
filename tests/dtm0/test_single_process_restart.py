...
class TestSingleProcessRestart:
    ...
    def perform_write_op(self, s3userinfo, bucket_name, object_name, clients, samples, size,
        ...
        if que:
            if s3bench.check_log_file_error(resp[1]):
                ...
                que.put(True, {'bucket': bucket_name, 'obj_name_pref': object_name,
                               'num_clients': clients, 'obj_size': size, 'num_sample': samples})
    ...
    def test_read_during_m0d_restart(self):
        ...
        resp = self.perform_write_op(s3userinfo=s3userinfo, bucket_name=bucket_name,
                                     ...
                                     log_file_prefix=test_41204_cfg['log_file_prefix'])
    ...
    def test_write_during_m0d_restart(self):
        ...
        proc_write_op = multiprocessing.Process(target=self.perform_write_op,
                                                args=(s3userinfo, bucket_name, object_name,
        ...
        resp = self.perform_read_op(s3userinfo,workload_info,True,True,True)
        assert_utils.assert_true(resp[0],resp[1])
