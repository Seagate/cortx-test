...
class TestControlPodSoftFailure:
    ...
    def test_45501(self):
        ...
        resp = self.ha_obj.check_cluster_status(self.node_master_list[0])
        assert_utils.assert_false(resp[0], resp)
        LOGGER.info("Step 2: Perform soft-failure of control pod & Checked cluster has failures.")
        time.sleep(self.csm_soft_delay)
    ...
    def test_45502(self):
        ...
        for failure in range(HA_CFG["common_params"]["short_loop"]):
            pod = secrets.SystemRandom().choice(pod_list)
        ...
        time.sleep(self.csm_soft_delay)
        LOGGER.info("Step 3: Verify background process for IAM user creation/deletion")
        event.clear()
        LOGGER.info("Waiting for background threads to join")
        thr_iam.join()
        iam_resp = tuple()
