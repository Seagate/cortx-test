...
class TestPodFailure:
    ...
    def test_degraded_deletes_safe_pod_shutdown(self):
        ...
        LOGGER.info("Step 1: Create 150 buckets and perform WRITEs with variable size objects.")
        buckets = [f"test-26444-bucket-{i}-{str(int(time.time()))}" for i in range(151)]
        ...
        LOGGER.info("Step 1: Sucessfully created 150 buckets & "
                    "perform WRITEs with variable size objects.")
        ...
        LOGGER.info("Step 5: Perform DELETEs on random 140 buckets")
        for _ in range(141):
        ...
        LOGGER.info("Step 5: Successfully performed DELETEs on random 140 buckets")
    ...
    def test_degraded_deletes_unsafe_pod_shutdown(self):
        ...
        LOGGER.info("Successfully created s3 account with name %s", self.s3acc_name)
        LOGGER.info("Step 1: Create 150 buckets and perform WRITEs with variable size objects.")
        buckets = [f"test-26644-bucket-{i}-{str(int(time.time()))}" for i in range(151)]
        for bucket in buckets:
            resp = self.ha_obj.ha_s3_workload_operation(
                s3userinfo=self.s3_clean, log_prefix=bucket, skipread=True, skipcleanup=True,
                nsamples=1, nclients=1)
        ...
        LOGGER.info("Step 6: Perform DELETEs on random 140 buckets")
        for _ in range(141):
            del_bucket = buckets.pop(self.system_random.randrange(len(buckets)))
            resp = s3_test_obj.delete_bucket(bucket_name=del_bucket, force=True)
            assert_utils.assert_true(resp[0], resp)
