    ...
    for item in ret.items:
        ...
        if "cortx" in item.metadata.name:
           ...
           for list_each_con in list_containers:
               ...
               resp = stream(v1.connect_get_namespaced_pod_exec, item.metadata.name, "default", container=list_each_con.name, command=exec_command_install_net_tool, stderr=True, stdin=False, stdout=True, tty=False, _preload_content=True)
               LOGGER.info("Executing netstat")
               resp = stream(v1.connect_get_namespaced_pod_exec, item.metadata.name, "default", container=list_each_con.name, command=exec_command_run_netstat, stderr=True, stdin=False, stdout=True, tty=False, _preload_content=True)
