...
api = Namespace('VM_Pool', path="/r2_vm_pool", description='VM Pool operations')


@api.route("/search", doc={"description": "Search for vm entries in MongoDB"})
...
class SearchPoolSystems(Resource):
    ...
    def get():
        ...
        count_results = mongodbapi.count_documents(json_data["query"], uri, read_config.db_name,
                                                   read_config.vm_pool_collection)
...
class UpdatePoolSystems(Resource):
    ...
    def patch():
        ...
        update_result = mongodbapi.update_document(json_data["filter"], json_data["update"],
                                                   ...
                                                   read_config.vm_pool_collection, upsert=False)
        if update_result[0]:
            return flask.Response(status=HTTPStatus.OK,
