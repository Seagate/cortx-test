...
#
"""This file contains s3 Object test scenario for s3 stability."""
...
class S3ObjectTestCase(unittest.TestCase):
    ...
    def test_checksum_part_file_positive_less_1mib(self, mock_getsize):
        ...
        expected_logs = [
            f'DEBUG:src.s3.s3api.s3_object_ops:Reading less than {1 * MIB}',
            f'DEBUG:src.s3.s3api.s3_object_ops:Reading {KIB} from starting offset {offset}']
        mock_getsize.return_value = file_size
        with self.assertLogs('src.s3.s3api.s3_object_ops', level='DEBUG') as logs:
    ...
    def test_checksum_part_file_positive_more_1mib(self, mock_getsize):
        ...
        data = os.urandom(file_size)
        expected_logs = [
            f'DEBUG:src.s3.s3api.s3_object_ops:Reading more than {1 * MIB}',
            f'DEBUG:src.s3.s3api.s3_object_ops:Reading {MIB} from starting offset {offset}',
            f'DEBUG:src.s3.s3api.s3_object_ops:Reading {MIB} from starting offset {10 + MIB}',
            f'DEBUG:src.s3.s3api.s3_object_ops:Reading {10} from starting offset {10 + (MIB * 2)}']
        mock_getsize.return_value = file_size
        with self.assertLogs('src.s3.s3api.s3_object_ops', level='DEBUG') as logs:
            with mock.patch("builtins.open", mock.mock_open()) as mock_open:
                mock_open.side_effect = [BytesIO(data)]
