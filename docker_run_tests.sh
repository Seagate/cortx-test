...
"$PYTHON_VAR" -u testrunner.py -te="$TEST_EXECUTION_NUMBER" -tp="$TEST_PLAN_NUMBER" -tg="$TARGET_NODE" -b="$BUILD" -t="$BUILD_TYPE" -d="$DB_UPDATE" --force_serial_run="$SEQUENTIAL_EXECUTION" -tt $TEST_TYPE
