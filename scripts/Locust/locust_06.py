#Put multiple different size objects in one buckets, get object count of that buckets and delete objects simultaneously from multiple clients/workers
...
class MyTaskSet(TaskSet):
    ...
    def delete_objects(self):
        ...
        if len(objects) > 1:
                start_time = time.time()
                obj = locust_utils.locust_var['s3'].Object(bucket_name, objects[0])
                obj.delete()
                print("Object Deleted Successfully : {} from bucket : {}".format(objects[0],bucket_name))
                total_time = int((time.time() - start_time) * 1000)
                events.request_success.fire(
                        request_type="delete",
                        name="delete_objects",
