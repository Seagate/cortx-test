...
class CortxCliS3AccountOperations(CortxCli):
    ...
    def create_s3account_cortx_cli(
        ...
        response = self.execute_cli_commands(cmd=command, patterns=["Password:"])[1]

        if "Password:" in response:
            response = self.execute_cli_commands(cmd=password, patterns=["Confirm Password:"])[1]
            if "Confirm Password:" in response:
                response = self.execute_cli_commands(cmd=confirm_password, patterns=["[Y/n]"])[1]
                if "[Y/n]" in response:
                    response = self.execute_cli_commands(cmd="Y", patterns=[account_name])[1]
    ...
    def show_s3account_cortx_cli(self, output_format: str = None) -> tuple:
        ...
        response = self.execute_cli_commands(cmd=show_s3accounts_cmd, patterns=["Account Name","account_email"])
        LOGGER.info("Response returned: \n%s", response)
    ...
    def delete_s3account_cortx_cli(self, account_name: str) -> tuple:
        ...
        response = self.execute_cli_commands(cmd=delete_s3acc_cmd, patterns=["[Y/n]"])[1]
        if "[Y/n]" in response:
            response = self.execute_cli_commands(cmd="Y", patterns=["Account Deleted"])[1]
    ...
    def reset_s3account_password(
        ...
        if "Password:" in response:
            ...
            if "Confirm Password:" in response:
                ...
                if "[Y/n]" in response:
                    response = self.execute_cli_commands(cmd=reset_password, patterns=[account_name, "cortxcli"])[1]
                    if account_name in response:
                        LOGGER.info("Response returned: \n%s", response)
                        return True, response
