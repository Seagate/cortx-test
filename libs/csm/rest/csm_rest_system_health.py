...
class SystemHealth(RestTestLib):
    ...
    def __init__(self):
        ...
        self.main_conf = ctpyaml.read_yaml("config/provisioner/provisioner_config.yaml")
    ...
    def get_health_summary(self):
        ...
        try:
            ...
            self._log.info(
                "Endpoint for health summary is {}".format(endpoint))
    ...
    def verify_health_summary(self, expected_response, verify_schema=True):
        ...
        :param verify_schema: Schema for verification of the response strings, defaults to True 
        ...
        try:
            ...
            self._log.info(
                "Expected status code: {}".format(expected_response))
            self._log.info("Actual status code: {}".format(
                response.status_code))
            if response.status_code == expected_response:
                self._log.info("Status code verification passed !")
                if verify_schema:
                    self._log.info("Verifying health summary schema...")
                    self._log.info("Expected status code: {}".format(
            ...
            raise CTPException(
                err.CSM_REST_VERIFICATION_FAILED, error.args[0])
        self._log.info(
            "Verification result of health summary : {}".format(result))
        return result
    ...
    def get_health_node(self, node):
        ...
        try:
            # Building request url
            self._log.info("Reading node {} health...".format(node))
            ...
            self._log.info("Endpoint to node health is {}".format(endpoint))
    ...
    def verify_health_node(self, expected_response, node, verify_schema=True):
        ...
        try:
            ...
            raise CTPException(
                err.CSM_REST_VERIFICATION_FAILED, error.args[0])
        self._log.info(
            "Verification result of health node : {}".format(result))
        return result
    ...
    def get_health_view(self, node):
        ...
        try:
            # Building request url
            self._log.info("Reading health view for node {}...".format(node))
            endpoint = self.config["health_view_endpoint"]

            # Adding parameters(if any) to endpoint
            endpoint = self._append_node_id_param(endpoint, node)
            self._log.info("Endpoint to Health View is {}".format(endpoint))
    ...
    def verify_health_view(self, expected_response, node, verify_schema=True):
        ...
        try:
            ...
            raise CTPException(
                err.CSM_REST_VERIFICATION_FAILED, error.args[0])
        self._log.info(
            "Verification result of health view : {}".format(result))
        return result
    ...
    def _get_node_ids(self, node):
        ...
        if node == "storage" or node == "all":
            ...
            raise CTPException(
                err.CSM_REST_VERIFICATION_FAILED, "Invalid Node Parameter")
        self._log.info("List of the node_ids are {}".format(node_ids))
        return node_ids
    ...
    def _append_node_id_param(self, endpoint, node):
        ...
        """
        """
        Append the node id parameter to the given endpoint
        """
        if endpoint is not None:
            node_id = self._get_node_ids(node)[0]
            self._log.info("node_id= {}".format(node_id))
    ...
    def _verify_response(self, node, response):
        ...
        if node == "":
            self._log.info(
                "As the node value selected is blank, All the possible node values will be verified...")
            node_ids = self._get_node_ids("all")
        else:
            self._log.info("Node value selected is : {}".format(node))
            node_ids = self._get_node_ids(node)
        self._log.info("Node IDs for verification are : {}".format(node_ids))

        response_json = response.json()

        cnt = len(response_json)
        self._log.info("Number of entries in response : {}".format(cnt))
        if cnt == 0:
            self._log.error(
        ...
        for node_id in node_ids:
            self._log.info(
                "Verifying the response for node : {}".format(node_id))
            node_summary = None
            for i in range(0, cnt):
                if node_id in response_json[i].keys():
                    self._log.info(
                        "Verifying health schema for node : {}".format(node_id))
                    node_summary = response_json[i][node_id]
                    break

            if node_summary is None:
                self._log.error("Node {} not found in response {}.".format(
            ...
            try:
                ...
                self._log.info(
                    "Health schema verification passed for node : {}".format(node_id))
                return True
            except:
                self._log.info(
                    "Health schema verification failed for node : {}".format(node_id))
                return False
