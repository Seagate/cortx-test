...
from eos_test.utility.utility import Utility
...
class SystemStats(RestTestLib):
    ...
    def get_stats(self, stats_id=None, panel=None, metrics=None,
                  from_time=None, to_time=None, interval=None, total_sample=None, op_format=None):
        ...
        try:
            ...
            self._log.info("Endpoint for reading stats is {}".format(endpoint))
    ...
    def verify_list(self, expected_list, actual_list):
        ...
        """
        self._log.info("Expected List : {}".format(expected_list))
        self._log.info("Actual List : {}".format(actual_list))
    ...
    def get_time_diff(self, to_time, from_time):
        ...
        rd = dateutil.relativedelta.relativedelta(to_time, from_time)
        rd.months = rd.months + rd.years * 12
        rd.days = rd.days + rd.months * 30
        rd.hours = rd.hours + rd.days * 12
        rd.minutes = rd.minutes + rd.hours * 60
        rd.seconds = rd.seconds + rd.minutes * 60
        return rd

    def expected_entry_cnt(self, to_time, from_time, interval=None, total_sample=None):
        ...
        """
        rd = self.get_time_diff(to_time, from_time)
        diff_sec = rd.seconds
        self._log.info("Time difference in seconds : {}".format(total_sample))
        if total_sample is not None:
            samples = total_sample
            self._log.info("Expected total samples : {}".format(total_sample))
        else:
            if interval is not None:
                samples = math.ceil(diff_sec/interval)
                self._log.info("Requires Interval or total Sample : {}".format(samples))
            else:
                samples = None
    ...
    def _add_parameters(self, endpoint, stats_id=None, panel=None,
        ...
        if panel is not None:
            params.append("panel={}".format(str(panel).lower()))
        if from_time is not None:
            params.append("from={}".format(str(from_time).lower()))
        if to_time is not None:
            params.append("to={}".format(to_time))
        if interval is not None:
            params.append("interval={}".format(interval))
        if total_sample is not None:
            params.append("total_sample={}".format(total_sample))
        if op_format is not None:
            params.append("op_format={}".format(op_format))
        if metrics is not None:
            for metric in metrics:
                params.append("metric={}".format(str(metric).lower()))
    ...
    def get_metrics(self):
        ...
        try:
            ...
            self._log.info("Metric list read is : {}".format(metric_list))
            return metric_list
