...
class RestS3Bucket(Base):
    ...
    def __init__(self):
        ...
        template_payload = Template(self.const.BUCKET_PAYLOAD)
    ...
    def create_and_verify_new_bucket(
        ...
        try:
            ...
            if (not response) or response.status_code != expect_status_code:
                self._log.error("Response is not 200")
                return False
            ...
            response = {"name": response[self.const.BUCKET_NAME]}
    ...
    def list_and_verify_bucket(
        ...
        try:
            ...
            if self.const.BUCKET not in response:
                self._log.error("Bucket key is not present")
                return False

            # Checking for not "no user" scenario
            if len(response[self.const.BUCKET]) == 0 or expect_no_user:
                self._log.warning("Buckets present till now is : {}".format(
                    len(response[self.const.BUCKET])))
                return len(response[self.const.BUCKET]) == 0 and expect_no_user

            # Checking format
            if not all(
                    self.const.NAME in key for key in response[self.const.BUCKET]):
                self._log.error("Invalid for mat of the json")
                return False

            return all(isinstance(value[self.const.NAME], str)
                       for value in response[self.const.BUCKET])
        except Exception as error:
            self._log.error("{0} {1}: {2}".format(
                self.exception_error,
...
class RestS3BucketPolicy(Base):
    ...
    def __init__(self, bucketname, ):
        ...
        template_payload = Template(self.const.BUCKET_POLICY_PAYLOAD)
        ...
        update_payload = template_payload.substitute(value=bucketname, s3operation='GetObject', effect='Allow',
                                                     principal='*')
        iam_principal_payload = Template(self.const.BUCKET_POLICY_PAYLOAD_IAM)
        multi_policy_payload = Template(self.const.MULTI_BUCKET_POLICY_PAYLOAD)
