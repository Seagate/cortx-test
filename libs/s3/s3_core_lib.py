...
class S3Lib:
    ...
    def __init__(self, access_key: str, secret_key: str, endpoint_url: str,
                 s3_cert_path: str, region: str, aws_session_token: str = None, debug: bool = False) -> None:
        """
    ...
    def put_object(self, bucket_name: str, object_name: str, file_path: str,
                   m_key: str = None, m_value: str = None):
        """
    ...
    def object_list(self, bucket_name: str) -> list:
        """
        Listing Objects.
        :param bucket_name: Name of the bucket.
        :return: response.
    ...
    def get_object(self, bucket: str, key: str, ranges: str) -> dict:
        """
    ...
    def put_object_with_tagging(
        ...
        if meta:
            response = self.s3_resource.Bucket(bucket_name).put_object(
                Key=object_name, Body=file_path, Tagging=tag, Metadata=meta)
        else:
            response = self.s3_resource.Bucket(bucket_name).put_object(
...
class BucketPolicy(S3Lib):
    ...
    def delete_bucket_policy(self, bucket_name: str) -> dict:
        """
        This function will delete the policy applied to the specified S3 bucket.
        :param bucket_name: Name of s3 bucket.
