...
class ProvDeployFFLib:
    ...
    def cortx_prepare(nd_obj: Node, build_branch: str, build_url: str):
        ...
        resp = nd_obj.execute_cmd(cmd=common_cmd.CMD_GET_PROV_INSTALL.format(build_branch),
                                  read_lines=True)
    ...
    def factory_manufacturing(self, nd_obj: Node, nd_no: int, node_config: SectionProxy, **kwargs):
        ...
        self.configure_network(nd_obj, network_trans)
        self.configure_storage(nd_obj, nd_no, node_config)
        self.configure_security(nd_obj, security_path)
        self.configure_feature(nd_obj, feature_conf)
        self.setup_node(nd_obj, nd_no)
    ...
    def deploy_3node_vm_ff(self, build_branch, build_url, deploy_config_file):
        """
        ...
        for nd_no, nd_obj in enumerate(node_list, start=1):
            self.deployment_prereq(nd_obj)
            self.cortx_prepare(nd_obj, build_branch, build_url)
        ...
        self.field_deployment_cluster(nd1_obj, hostnames, srvnodes, deploy_cfg, build_url)

        LOGGER.info("Starting the post deployment checks")
        self.post_deploy_check(nd1_obj)
