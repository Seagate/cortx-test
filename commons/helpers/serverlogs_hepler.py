...
log_destination = "/temp/logs"              # @TODO change this path accordingly
remotepath = "10.237.65.160:/root/mahesh"   # @TODO change this path accordingly
file_exention = ".log"
filenameformat = "cortx_"                   # TODO change this path accordingly

#@TODO change this path accordingly
file_path_dict = {"s3":"/home/s3path","motr":"/home/motrpath","ras":"/home/raspath", "ha":"/home/hapath", "prov":"/home/provpath"}
node_ip_dict ={"node1":"10.237.65.160","node1":"10.237.65.160"}

class node_data:        #@TODO - visit this again
    def __init__(self):
        self.ip = Null
        self.uname = "root"
        self.passwd = "seagate1"

def get_node_details(node_name):    #@TODO- visit again
    node_obj = node_data
    node_obj.ip = node_ip_dict.get(node_name)
    node_obj.uname = "root"
    node_obj.passwd = "seagate1"
...
def split_file_for_timestamp(st_time, end_time, filename, filepath,test_id):
    ...
    for line in logfile:
        ...
        if timestamp >= dummy_start_time and timestamp <= dummy_end_time:
            newfile.write(line)
...
def process_and_copy_file(st_time, end_time, file_name, file_path, localpath,test_id,sftp):
    ...
    nodepath = "{}/{}".format(filepath, filename)
    sftp.get(nodepath, log_destination) #@TODO - check sftp.get(), this call might be wrong
    ...
    # 2. Copy file from node to remote server
    sftp.put(localpath=newfilepath, remotepath=remotepath)
...
def collect_logs(st_time, end_time, file, node, test_id):
    ...
    if file is "all":
        for fname in file_list:
            file_name = "{}{}{}".format(filenameformat, fname,extention)
        ...
        file_name = "{}{}{}".format(filenameformat, file, extention)
...
def collect_logs_fromserver( st_time, file_type='all', node='all', end_time = current_time,test_id): # @TODO test_id ->suffix
    # Collect logs for all nodes
    if node is 'all':
        for node_name in node_list:
            response = collect_logs(st_time, end_time, file_type, node, test_id)
    # collect from one node only
    else:
        response = collect_logs(st_time, end_time, file_type, node, test_id)

# @ TODO - support bundle function